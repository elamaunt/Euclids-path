...
pigeonhole — пижонхол, принцип ящиков (см. [словарь](GLOSSARY.md)).
...
1. `AmbientLegal`: берём свидетель — конкретный объект, удостоверяющий утверждение (см.
   [словарь](GLOSSARY.md)), — `N₀ = N`. Он положителен (из `1 < N`), не превосходит потолка
...
см. [словарь](GLOSSARY.md)), отделяющий локальную факторизацию от глобального запуска двигателя.
