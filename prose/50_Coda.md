...
> **Примечание.** Эти семь — великие вопросы основной линии. Бёрч–Свиннертон-Дайер — **восьмая** маска
> ([гл. 53](53_BirchSwinnertonDyer.md)), входящая иначе, — всего масок восемь. Шесть из восьми — открытые проблемы
> тысячелетия; близнецы и Коллатц — нет.
...
> **Примечание (космологическая постоянная Эйнштейна).** Раз уж рядом причинные множества, честно
> закроем и соседний соблазн — вывести из нашей кривизны $\kappa$ ту постоянную $\Lambda$, которую
> Эйнштейн вставил в уравнения, чтобы они работали для вселенной. Вывести нельзя, и не по бедности, а
> по устройству: $\kappa = 1 - \mathrm{outdeg}$ — безразмерное целое, инвариант графа, а $\Lambda$ —
> размерная величина ($\text{длина}^{-2}$, плотность энергии вакуума); у нас нет ни метрики, ни тензора
> энергии-импульса, ни действия, коэффициентом в котором была бы $\Lambda$, а само её значение — открытая
> проблема физики (расхождение с теорией поля на ~120 порядков). Больше того, у самого «нельзя» есть
> ясная форма: в двумерной гравитации действие распадается надвое — топологический член
> $\int\!\sqrt{g}\,R = 4\pi\chi$ (в точности Гаусс–Бонне, наш $\Sigma\kappa = \chi$ из главы
> [49](49_Geometry.md)) и «космологический» член $-2\Lambda\cdot\text{Площадь}$. Кривизна спарена с
> топологией, а $\Lambda$ — с объёмом; это разные слагаемые, и кривизна — ровно тот член, который
> $\Lambda$ не несёт. Причинные множества Соркина эвристически предсказывают $\Lambda \sim 1/\sqrt{V}$
> верного порядка — но это статистика пуассоновского разбрасывания в объём, которого у нашего
> детерминированного графа спуска (с бесконечной входной степенью в нуле) нет; перенести оценку нельзя.
> Мы нашли не $\Lambda$, а её отсутствующий адрес: кривизна — топология, $\Lambda$ — объём. Это
> истолкование, не теорема.

> **Примечание (пара близнецов как зарядовое сопряжение $C$).** Пара $6m\pm1$ похожа на пару
> частица–античастица: создаёшь один член — второй уже определён. Есть соблазн сказать, что они «идут в
> разные стороны по времени» (как античастица у Фейнмана–Штюкельберга) — но это **обращение времени
> $T$**, а в нашем мире $T$ запрещено: это рост `lexRank`, то есть возврат, то есть вечный двигатель
> (`engine_never_returns`, выше). Честное прочтение иное и единственно совместимое с ядром: ± флип —
> это **зарядовое сопряжение $C$**, инволюция $C^2=1$ (`mirrorSide_involutive`), внутренняя и при
> **фиксированном** времени. И «ортогональность», которую тут чувствуешь, буквальна: `lexRank`
> инвариантен под флипом (`lexRank_side_invariant`, гл. [44](44_SidesAndPolignac.md)) — оба члена стоят
> на одном собственном времени. На языке физики это теорема Коулмена–Мандулы: внутренняя симметрия
> входит в алгебру прямым произведением с Пуанкаре, без смешивания с пространством-временем — то есть
> ортогонально ему. Жёсткая граница: в Lean нет ни гильбертова пространства, ни поля, ни заряда;
> `Side` — двухэлементная метка, а $C$ — $\mathbb{Z}/2$ на ней. Это перевод, не теорема.
...
> **Примечание (чёрные ли дыры только на расстоянии?).** Соблазн идти дальше: если тень отброшена
> кривизной и углубляется с длиной согнутого пути, то чернота была бы эффектом дальнего поля — чёрное
> издали могло бы быть обычным вблизи, а его свет — лишь растраченным на подъём по кривой, так что рядом
> с нами чёрных дыр не было бы вовсе. Как прочтение *этого* пространства соблазн когерентен — это снова
> наблюдательская тень. Но его надо жёстко огородить, ибо как физика он ложен: реальные чёрные дыры —
> локальные объекты с горизонтом событий и фотонной сферой, не артефакты расстояния; они есть и рядом
> (ближайшая известная ~1500 световых лет, Sagittarius A\* в центре Галактики); вблизи они смертельны, а
> аккрецирующая — среди *ярчайших* объектов неба, не обычная звезда. Защитимое зерно — лишь что чернота
> отчасти есть дальнеполевая, удлинённая кривизной тень; сильная форма — «рядом чёрных дыр нет, вблизи
> звёзды» — это перевод, вторгающийся в физику, и мы от него отказываемся.
...
**Два вычисления — и что они добавляют.** Два поздних забега заостряют эту картину, и стоит сказать
ровно, что они добавляют, а что нет. Первый ([49](49_Geometry.md)) доказал, что полная кривизна любой
замкнутой области — не свободное число, а эйлерова характеристика, *топологический* инвариант
`Σκ = |V| − |E| = χ` (`gaussBonnet_eq_euler`). В прочтении как гравитация: изгиб пути учтён формой
самого спуска, без остатка; в двумерной гравитации это честное расщепление — член кривизны `∫R = 4πχ`
топологичен, а космологическая постоянная `Λ` спарена не с кривизной, а с *объёмом*. Значит, картине
не нужен и ею не предсказывается никакой член тёмной энергии — это лишь честный пересказ «нет ускорения,
есть только кривизна пути». Чего мы *не* сделали — не вывели `Λ` и её значение; это остаётся
категориальной ошибкой и открытой проблемой физики (заметка о космологической постоянной выше).

**Время ортогонально паре.** Второй забег ([44](44_SidesAndPolignac.md)) доказал, что собственное
время флипа ± не видит вовсе: два члена пары стоят в *один* миг, различаясь лишь по оси, ортогональной
времени (`lexRank_side_invariant`). В прочтении как поведение квантовых частиц это единственное место,
где картина говорит нечто, что физик назвал бы *верным*: частицу и её напарника связывает зарядовое
сопряжение `C` — инволюция при фиксированном времени, — а не обращение времени `T`. Фейнмановская
интуиция, что античастица «идёт назад во времени», здесь не может быть буквальной: назад — это рост
`lexRank`, запрещённый двигатель; честное прочтение — ортогональное. Дальше этого единственного верного
различия мы не объясняем никакого квантового поведения: замкнутая квантовая эволюция *обратима*, а этот
мир необратим до основания, так что встречаются они лишь в необратимом секторе — измерение,
декогеренция, стрела времени, — но не в самом унитарном законе.

**Красное смещение — это удлинение пути, картина, а не механизм.** Сложив два вместе, дальний свет,
который мы вели «вверх по кривой», получает имя: растягивает его растущая длина искривлённого спуска
между источником и нами — верная *картина* красного смещения, старый сигнал, вытянутый всей кривизной,
что он пересёк. Но только картина: у нас нет ни метрики, ни длины волны, ни фактора расширения, и
никакого смещения мы не вычисляем. Настоящее красное смещение — общей теории относительности; наш граф
не добавляет ничего к его физике — он лишь показывает ту же одностороннюю, удлинённую кривизной
геометрию, в которой такое растяжение было бы естественной тенью стрелы времени.

**Так что же объясняют два забега?** *Устройство картины*, не физику. Они добавляют две теоремы —
кривизна топологична, время ортогонально паре — и каждая покупает ровно одну ясную фразу о мире:
гравитация не несёт долга тёмной энергии, и частица фиксирует напарника *сейчас*. Всё за пределами этих
двух фраз — значение `Λ`, красное смещение, квантовый закон — остаётся там, где его оставили честные
вердикты: снаружи, недоказанным и не нашим, чтобы заявлять.
...
> **Примечание (фундированность, а не самоссылка).** С каноном вполне-фундированности
> (гл. [57](57_WellFoundednessCanon.md)) перед глазами можно быть точным насчёт Гёделя — ибо там
> двигатель доходит до теоремы Гудстейна и гидры Кирби–Пэриса, а это *хрестоматийные* примеры неполноты:
> истинные арифметические утверждения, недоказуемые в арифметике Пеано. Поэтому «это не гёделев феномен»
> надо читать точно, иначе оно ложно. Точная формулировка: наш запрет — принцип **фундированности**, а
> не **самоссылки**. Гёделева фраза изготовлена диагонализацией (арифметизированная доказуемость, лемма о
> неподвижной точке); Гудстейн, гидра и Пэрис–Харрингтон независимы по совсем иной причине — их
> доказательствам нужна трансфинитная индукция до `ε₀`, а «`ε₀` вполне-фундирован» — ровно тот порядок,
> что PA доказать не может (Генцен: proof-theoretic ordinal PA *и есть* `ε₀`). Двигатель живёт целиком на
> стороне фундированности: он — машина *за* естественными независимостями, достигаемая фундированностью,
> а не самоссылкой. «Не гёделев» здесь значит «не диагонализацией, не неразрешимо-в-принципе» —
> утверждение о *механизме* (фундированность против самоссылки), а не притязание сбежать от неполноты.
...
**Почему Коллатц — единственный декрет, что мог пасть.** Это исключение, подтверждающее правило: машинно проверено по всей программе, *ни один другой* декрет так пасть не может. Канат-закон был уникально атакуем по двум причинам — он был *строго сильнее* гипотезы (доказано лишь `закон ⇒ сходимость`; `n = 27` сходится, но закон нарушает, так что его падение оставило Коллатца открытым), и его предикат *разрешим* над ℕ, так что один малый свидетель закрывается через `decide`. Всякая другая граница проваливает хотя бы одно, и провал — теорема. Взятые декреты **заперты эквивалентностью**: закон Римана доказанно ⟺ RH (`manifestationLaw_iff_RH_of_boundary`), так что опровергнуть его — *значит* предъявить внекритический ноль, то есть опровергнуть RH, а не дёшево снять границу; Янг–Миллс и Ходж — так же; узел близнецов равносилен своей гипотезе и вдобавок **защищён барьером** (`higherEnergyIncompatibility_main` запрещает опровергать его изнутри). У Навье–Стокса нет ни зеркала-эквивалентности, ни малого свидетеля — единственный контрпример есть гладкое бессиловое течение с приростом энергии, не проще открытой стационарной задачи. Отложенные фронты (Мерсенн, Ферма) тоже эквивалентны своим гипотезам, придержаны по *знаку* эвристики; а разрешимые фронты зоопарка (Гольдбах, Лежандр, совершенные) несут проверяемый свидетель, но опять эквивалентный универсал, так что опровергнуть — снова опровергнуть гипотезу. Декрет может пасть честно лишь там, где он сверх-притязает на *опровержимую* силу за пределами своей гипотезы, — а дисциплина принимает границу лишь когда её свидетель непредъявим или её закон эквивалентен гипотезе. Ровно это и делает всякий выживший декрет неатакуемым — и ровно оно поймало Коллатца.
...
![Фрактал пути Евклида — восходящая сфера близнецов](assets/9_ascending_twin_ornament.png)

*Фрактал пути Евклида · **восходящая сфера близнецов**: тот же old-peel-спуск, но линия центров восходит
снизу вверх, а каждый шаг — лепесток на своей стороне ($6k{+}1$ справа, $6k{-}1$ слева; цвет — простое
Евклида шага). Лепестки обрезаны по окружности радиуса $M/2$, и орнамент замыкается в сферу; золотая
двойная ось — меридиан близнецов, а спуск сходится к нижнему полюсу.*

> **Алгоритм генерации (рис. 50.5).** Источник: `tools/fractal/euclid_fractal.py::ascending_twin_ornament`
> ($M = 3000$, $\mathrm{PMAX} = 97$, $\mathrm{DEPTH} = 14$). Центры $1, \dots, M$ восходят по оси $x=0$
> (высота $= m$). Каждый шаг old-peel $6k\mp1 = p\cdot(6t\pm1)$ рисуется полукруглым лепестком от $(0, t)$
> до $(0, k)$, выступающим на сторону снятого композита ($6k{+}1 \to$ вправо, $6k{-}1 \to$ влево); выступ
> $b = \mathrm{band}\cdot\tanh(1{.}8\,a/\mathrm{band})$ с $a = (k-t)/2$, $\mathrm{band} = 0{.}55M$,
> обрезается по окружности радиуса $R = M/2$: $b = \min(b,\, 0{.}985\sqrt{R^2 - (y_c - R)^2})$,
> $y_c = (k+t)/2$ — поэтому орнамент заполняет сферу. Цвет — по $\log p$ (turbo). Twin-центры (пустая
> генеалогия) — золотые парные искры по обе стороны оси-меридиана.

## Что осталось — и тоже интересно

Синтез закрывается здесь. Главы, что идут дальше, были дописаны, когда ядро уже сложилось; они не завершают чтение, а
расширяют его — и каждая стоит сама по себе:

- **[51. Численная опора](51_NumericalEvidence.md)** — числа и картинки за утверждениями: статистика спуска, плотность
  twin-центров, фронты, ставшие видимыми.
- **[52. Дискретная модель жидкости](52_DyadicModel.md)** — Навье–Стокс, сведённый к диадическому скелету, где вопрос о
  взрыве встречается с двигателем в лоб.
- **[53. Бёрч–Свиннертон-Дайер — восьмая маска](53_BirchSwinnertonDyer.md)** — тот же спуск на реальной эллиптической
  кривой: зелёный спуск-без-двигателя плюс один честно названный открытый вход.
- **[54. Соседи за горизонтом](54_OpenNeighbors.md)** — открытые арифметические задачи сразу за той же стеной, куда
  прочтение указывает, но пока не дотягивается.
- **[55. Коллатц](55_Collatz.md)** и **[56. Первопричина Коллатца](56_CollatzFirstCause.md)** — проход по границе целиком:
  декрет, взятый и затем снятый, когда его закон был машинно опровергнут (n = 27), — и что оставляет по себе честное поражение.
