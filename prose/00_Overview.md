...
> `prose/NN_*.md` (парная проза, 00→39) и модули `EuclidsPath/Engine/*.lean` (машинная проверка).
...
> `Engine/CausalClosureAxiom.lean`, ровно 35 таких деклараций, утечек нет — верификатор
...
ровно 35 AXIOM-TAINTED деклараций (34 в карантинном модуле + задокументированное следствие
`higherEnergyIncompatibility_twins`), верификатор репортит каждую. Третьей (P/NP) границы НЕТ —
намеренно и машинно (трилемма главы 39); §11 карантина фиксирует, что декрет УЖЕ говорит на
языке P/NP: на своём масштабе `A ≥ 5` он платит все сертификаты. Непротиворечивость расширенной
теории ⟺ неопровержимость узла и RH в базе (растяжки — узел / off-critical нуль / ¬RH / закон /
несжимаемость на всех масштабах); интернализация первопричины невозможна — двигатель
...
буквально — на голом ядре Lean; главы 33–39 разворачивают первопричину (теперь с двумя
