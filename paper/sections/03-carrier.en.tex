...
\begin{theorem}[a shared divisor of the sides divides two]\label{thm:twin_sides_shared_dvd_two}
\green{}\ Let $m\ge 1$ and let $p\mid(6m+1)$ and $p\mid(6m-1)$. Then $p\mid 2$.
\leantag{twin\_sides\_shared\_dvd\_two}
\end{theorem}
...
\begin{theorem}[no prime above two divides both sides]\label{thm:no_large_shared_divisor}
\green{}\ Let $m\ge 1$ and $p>2$. Then $p\mid(6m+1)$ and $p\mid(6m-1)$ cannot hold simultaneously.
\leantag{no\_large\_shared\_divisor}
\end{theorem}
...
\begin{theorem}[the determinant law for a rank-$(3,3)$ centre]\label{thm:det_law_rank33}
\green{}\ Let $1\le m$, $6m-1=abv$ and $6m+1=qrs$. Then $qrs=abv+2$.
\leantag{det\_law\_rank33}
\end{theorem}
...
\begin{theorem}[the determinant law in product coordinates]\label{thm:det_law_CD}
\green{}\ Over $\Z$: if $Cb=6m-1$ and $Dr=6m+1$, then
\begin{equation}\label{eq:detCD}
Cb-Dr=-2.
\end{equation}
\leantag{det\_law\_CD}
\end{theorem}
...
\begin{theorem}[the determinant collapses along a ray]\label{thm:det_collapse}
\green{}\ Let $u_1 v+2=r_1 s$, $u_2 v+2=r_2 s$, $u_2-u_1=sK$ and $s\ne 0$. Then
\begin{equation}\label{eq:detcollapse}
u_2 r_1-u_1 r_2=2K.
\end{equation}
\leantag{det\_collapse}
\end{theorem}
...
\begin{theorem}[the Euclidean descent identity]\label{thm:euclid_descent_identity}
\green{}\ Given $abv+2=qP$, $P=v^2+\Delta$ and $ab=qv+\Xi$, then
\begin{equation}\label{eq:euclid-descent}
q\,\Delta-v\,\Xi=2.
\end{equation}
\leantag{euclid\_descent\_identity}
\end{theorem}
...
\begin{theorem}[the small-determinant collapse across a bridge]\label{thm:small_det_collapse}
\green{}\ Given $a\ne 0$, $qs\ne 0$ and the bridge equations above,
\begin{equation}\label{eq:smalldet}
B_2 Q_1-B_1 Q_2=12\,h.
\end{equation}
\leantag{small\_det\_collapse}
\end{theorem}
...
\begin{theorem}[the two migrates to the opposite side]\label{thm:two_carry_opposite}
\green{}\ If $U=6n+\varepsilon$, then
\begin{equation}\label{eq:two-carry}
6n-\varepsilon=U-2\varepsilon.
\end{equation}
\leantag{two\_carry\_opposite}
\end{theorem}
...
\begin{theorem}[no small prime divides the product side]\label{thm:no_small_divisor}
\green{}\ Let $b,v$ be primes with $A<b$, $A<v$, and $p$ prime with $p\le A$. Then $p\nmid(b\cdot v)$.
\leantag{no\_small\_divisor}
\end{theorem}
...
\begin{theorem}[any obstruction lands on the opposite side]\label{thm:obstruction_on_opposite}
...
\end{equation}
\leantag{obstruction\_on\_opposite}
\end{theorem}
...
\begin{theorem}[exclusivity kills the diagonal]\label{thm:exclusive_diag_zero}
...
\begin{theorem}[exclusivity forbids a backward move]\label{thm:exclusive_no_backward}
...
\end{equation}
\leantag{exclusive\_no\_backward}
\end{theorem}
