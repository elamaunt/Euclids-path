...
import EuclidsPath.Engine.CollatzTugOfWar   -- Коллатц: канат vs двигатель, window_budget ⟹ спуск при перевесе; ГЕРОЙ reaches_one_of_countingLaw; вход RopeCountingLaw 🔴; опровержение=вечный двигатель 🟢
import EuclidsPath.Engine.CollatzFirstCause -- 🟡 ПОТРЕБИТЕЛЬ: закон каната = collatzBoundary первопричины ⟹ collatz_from_firstCause; растяжки; решение заперто за двигателем 🟢; «проверка, а не вывод»
...
import EuclidsPath.Engine.BSDFront -- ВОСЬМАЯ МАСКА: тень Бёрч–Свиннертон-Дайера. Роль движка ИНАЯ — он сам метод СПУСКА, доказывающий алгебраический ранг: bsd_descent_has_no_engine (спуск по высоте на РЕАЛЬНЫХ точках W.toAffine.Point не имеет двигателя = терминация Морделла–Вейля, реюз UniversalEngine; ссылки на реальные Height/Northcott/fg_of_descent/LSeries); паритет-мост bsd_parity_is_rankParity к Лиувиллю (rank-parity узел). 🔴 честно открыто: AnalyticRank (порядок нуля РЕАЛЬНОЙ WeierstrassCurve.LSeries в s=1), WeakBSD (rank=ord L) — аналитика вне mathlib, движок её НЕ закрывает. Трилемма: честной границы НЕТ (декрет паритета вакуумен над заглушкой корневого числа) — фолбэк 🔴, аксиома нетронута, таинт 47. НЕ доказательство BSD
import EuclidsPath.Engine.ChowlaFront -- СОСЕД (приложение D): тень Чоулы/Сарнака — rank-parity узел. 🟢 реальный Лиувилль λ=(−1)^Ω: λ²=1, диагональ корреляции = x (совершенная само-корреляция паритета), флип паритета (реюз RiemannLiouville), liouvilleSum_eq_L; «отклонение = коллапс паритета в одну чётность». 🔴 ChowlaConjecture (корреляции o(x)), SarnakConjecture — открыты (Тао: лишь лог/нечётный случай). НЕ доказательство; таинт 47
import EuclidsPath.Engine.AbcFront -- СОСЕД (приложение D): тень abc. 🟢 РЕАЛЬНЫЙ Polynomial.abc (Мейсон–Стотерс ДОКАЗАН) — полиномиальный abc = степенная граница через радикал = «нет бесконечного побега» (конечность); реальные Nat/Int.radical. 🔴 AbcConjecture (интегральный) открыт (заявка Мочидзуки не принята сообществом). НЕ доказательство; таинт 47
import EuclidsPath.Engine.BealFront -- СОСЕД (приложение D): тень Бил/Ферма–Каталан — чистый Ферма-спуск = движок. 🟢 РЕАЛЬНЫЙ Polynomial.flt_catalan (ДОКАЗАН, спуск по char p) + fermatLastTheoremFour/Three (спуск) + descent-без-двигателя (реюз EPMI/UniversalEngine). 🔴 BealConjecture, FermatCatalanConjecture открыты (Дармон–Гранвиль: лишь фикс. показатели). НЕ доказательство; таинт 47
import EuclidsPath.Engine.LehmerFront -- СОСЕД (приложение D): тень Лемера/меры Малера — высота-конечность как в BSD. 🟢 РЕАЛЬНЫЕ finite_mahlerMeasure_le (Нортокотт: огр. степень+мера ⟹ конечно = нет бесконечного высотного спуска) + pow_eq_one_of_mahlerMeasure_eq_one (Кронекер: мера 1 ⟺ корни из единицы) + descent-без-двигателя. 🔴 LehmerConjecture (острая константа c>1) открыта. НЕ доказательство; таинт 47
import EuclidsPath.Engine.LandauFront -- СОСЕД (приложение D): тень Landau n²+1 (зоопарк, зеркало SideInfinitude). 🟢 мост неограниченность⟹бесконечность + факт oddLandauPrime_even_k (нечётное k ⟹ k²+1 чётно ⟹ единственное простое = 2). 🔴 Landau4thUnbounded (Харди–Литтлвуд) открыт (Иванец: ≤2 множителя). НЕ доказательство; таинт 47
