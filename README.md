...
потребитель — `Engine/CollatzFirstCause` (приложения [E](prose/E_Collatz.md)–[F](prose/F_CollatzFirstCause.md)).
...
| D | **Соседи за горизонтом (5 теней)** | [D](prose/D_OpenNeighbors.md) | `Engine/{Chowla,Abc,Beal,Lehmer,Landau}Front` | 🟢 РЕАЛЬНЫЕ якоря mathlib (Мейсон–Стотерс, Polynomial.flt_catalan, Нортокотт+Кронекер, Дирихле, Лиувилль); 🔴 сами гипотезы (Чоула/abc/Бил/Лемер/Landau) открыты; декретов нет; НЕ решения |
| E | **Коллатц: двигатель на ограниченном топливе** | [E](prose/E_Collatz.md) | `Engine/CollatzEngine`, `Engine/CollatzFuel`, `Engine/CollatzTugOfWar` | 🟢 законы двигателя/каната + бюджет окна ⟹ спуск + «опровержение = вечный двигатель»; 🔴 вход `RopeCountingLaw` |
| F | **Первопричина Коллатца: заперто за двигателем** | [F](prose/F_CollatzFirstCause.md) | `Engine/CollatzFirstCause`, `Engine/CausalClosureAxiom` §18 | 🟡 ЧЕТВЁРТАЯ граница (закон каната) ⟹ `collatz_from_firstCause`; 🟢 эпистемика «проверка, а не вывод»; декрет, возможно, переплачивает |
