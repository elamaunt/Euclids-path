...
> `prose/NN_*.md` (парная проза, 00→42) и модули `EuclidsPath/Engine/*.lean` (машинная проверка).
...
> (первопричина `0 → 1` с ТРЕМЯ границами — twin-узел, римановский закон манифестации и
> НС-гейт-закон энергобаланса; карантин
> `Engine/CausalClosureAxiom.lean`, ровно 43 такие декларации, утечек нет — верификатор
...
**P vs NP — локальная архитектура; сепарация в ранговой модели — 🟢 теорема (глава 39).**
🟢: верификация легка всегда (`verificationEasy_always`), локальный успех ⟺ семантический узел
близнецов (`localPSuccess_iff_semanticFlowLedgerCollisionResolves`), на малом масштабе `A ≤ 4`
несжимаемость безусловна (`concrete_localSearchIncompressible_smallScale`).
✳️ **Прочтение «NP = полная оплата сертификатов ранга, P = ранго-быстрый проезд» — теорема**:
`pnp_rank_separation_smallScale` 🟢 и `concrete_localPSuccess_iff_fullPayment` 🟢 (глава 39);
раскол по масштабам — декрет на `A ≥ 5` сам платит все сертификаты (`decreedScale_fullPayment` 🟡);
третьей границы декрета НЕТ машинно (трилемма: два кандидата опровержимы, третий вакуумен).
⚠️ **Вакуумность №4:** decider-gated extraction-фронты классически пусты (`PDecider` свободен) —
их separation-выводы вакуумны. Классическое разделение
(`classicalSeparation_of_localIncompressible`) условно на InP-gated мост; фреймы пластичны
(`allPFrame`/`constantsFrame`) — это **не** доказательство P ≠ NP, и оно не объявляется.
...
✳️ **Гладкость через каскад + взятый интеграл (глава 41):** сингулярность = сингулярный каскад =
вечный двигатель; `noSingularCascade_of_energyBalance` 🟢 — точек разрыва каскадного типа нет
(суррогат ≠ C^∞ — раскрыто); интеграл взят — `energy_identity_of_energyBalance` 🟢 (тождество
равенством) и `isNSSolution_integral_form` 🟢 (mild-форма, E3-значный FTC). Пятой границы декрета
НЕТ — трилемма главы 41 (cookedFlow / нулевое решение / кованый профильный каскад).

**Янг–Миллс — структурный результат, не Клэй (глава 40).** 🟢: безмассовость = вечный двигатель
(halving-лестница — тот самый ℝ-контрпример каскадного предупреждения); **квантованность спектра ⟹
масс-щель** (`massGap_of_quantizationLaw`: лестница + ранг = ℕ-спуск, убит EPMI). Четвёртой границы
декрета НЕТ — трилемма машинна (универсал опровержим, экзистенциал вакуумен, риманово зеркало
несовместимо с принятой границей — лестница предъявима, в отличие от нуля; коллапс
`quantizationLaw_iff_massGap` — закон ⟺ щель зелёно без границы). Мир декрета гэпнут в языке
поставок (`decreedScale_no_deviationSupply` 🟡). 🔴 вход — data-anchored построенный YM-спектр.

**Ходж — структурный результат, не Клэй (глава 42).** 🟢: класс Ходжа = квантованный
(p,p)-заряд (ℕ-высота), цикл = оплата; **двигатель (цепь неоплаченного спуска) мёртв
БЕЗУСЛОВНО** (`isEmpty_unpaidDescentChain` — квантование встроено в модель, асимметрия с ЯМ
раскрыта); `hodgeProperty_of_descentLaw` — закон спуска ⟹ гипотеза модели (сильная индукция
по высоте). Коллапс `descentLaw_iff_hodgeProperty` (обратная сторона вакуумна — раскрыто) —
шестого поля НЕТ; трилемма главы 42: универсал опровержим (cookedUnpaid), экзистенциал доказан
вообще без аксиом, chain-форма вырождена в зелёную (V2′), манифестация над предъявимым классом
несовместима с границей. В mathlib теории Ходжа нет; 🔴 вход — `DescentLaw` для настоящих
(p,p)-классов.
...
twin-узел (`causalBoundary`), римановский закон манифестации (`riemannBoundary`, глава 38) и
НС-гейт-закон энергобаланса (`nsBoundary`, глава 41 — ДОБИТИЕ: пережил расширенную трилемму,
две ковки убиты гейтами; декрет, возможно, переплачивает — раскрыто);
ровно 43 AXIOM-TAINTED декларации (42 в карантинном модуле + задокументированное следствие
`higherEnergyIncompatibility_twins`), верификатор репортит каждую. Третьей (P/NP) границы НЕТ —
намеренно и машинно (трилемма главы 39), четвёртой (Янг–Миллс), пятой (НС) и шестой (Ходж)
тоже (трилеммы глав 40–42); §11–§14 карантина фиксируют, что декрет УЖЕ говорит на языках
P/NP и ЯМ (на своём масштабе `A ≥ 5` он платит все сертификаты и отказывает поставке
отклонений — гэпнут), и несут по одной растяжке на НС и Ходж. Непротиворечивость
расширенной теории ⟺ неопровержимость узла и RH в базе (растяжки — узел / off-critical нуль /
¬RH / закон / несжимаемость на всех масштабах / поставка на всех масштабах /
манифестационные законы НС и Ходжа); интернализация
первопричины невозможна — двигатель (`no_internalSelfDerivation_step00CausalClosure`, 🟢).
...
буквально — на голом ядре Lean; главы 33–42 разворачивают первопричину (теперь с ТРЕМЯ
границами — близнецы, Риман и НС-гейт) и классические фронты.

## Философское отступление: один физический закон под семью задачами

Стоит проговорить то, что связывает всю программу, потому что связь эта — не
литературный приём, а её несущая конструкция. В основании лежит **самый твёрдый
из физических законов — невозможность вечного двигателя**: нельзя черпать работу
из ничего, нельзя бесконечно спускаться, не платя. У Евклида-Ферма это
`no_infinite_descent`: строго убывающая цепь натуральных высот обрывается. И
вся программа — открытие, что семь великих вопросов суть один и тот же запрет,
надетый на разные объекты.

Близнецы: конечность близнецов загнала бы бесконечный поток чистых стартов в
конечную клетку — rigid-цикл, двигатель. Риман: нуль вне прямой — неустойчивая
мода скрытого квантового спектра (Гильберт–Полиа), самоусиливающееся колебание,
двигатель. Янг–Миллс: безмассовый спектр — башня сколь угодно дешёвых
возбуждений вакуума, даровая перекачка, двигатель, а масс-щель — плата, которую
вакуум берёт за первое возбуждение. Навье–Стокс: сингулярность — каскад
Колмогорова, дошедший до бесконечно малых масштабов за конечное время,
бесконечная энергия из конечной, двигатель, а вязкость — верёвка, что всегда
успевает у дна. Ходж: неоплаченный квантованный заряд — вечный регресс всё
меньших знаменателей, двигатель на целочисленной решётке. P/NP: полная оплата
всех сертификатов дороже ранго-быстрого проезда — термодинамическая цена
информации. Коллатц (вне репо): затухающий дрейф, чья средняя сведена, а дно
поглощающе.

Один корень, семь ветвей. И в каждой — одна и та же честная стена: структурная
половина доказывается зелёно (где есть запрет двигателя, там нет отклонения), а
последний шаг — привязка к настоящему объекту (спектру КТП, гамильтониану простых,
решению Лерэ, `(p,p)`-классам, машине Тьюринга) — либо принимается декретом
первопричины под честно раскрытую цену, либо остаётся 🔴-входом. Программа не
решает проблемы тысячелетия; она показывает, что все они — тени одного
физического «нельзя», и предъявляет ровно ту часть, что от конкретного объекта не
зависит. Математика здесь оказывается термодинамикой, переписанной на языке
рангов: там, где физик говорит «даровой энергии не бывает», формалист доказывает
`no_infinite_descent` — и этого одного хватает, чтобы семь раз подойти вплотную к
границе познаваемого.
