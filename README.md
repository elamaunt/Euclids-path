...
`step00FirstCause` (четыре жёлтые границы декрета), либо остаётся зелёной условной теоремой, либо —
...
flowchart TD
    ...
    ROOT --> FC{{"step00FirstCause · ЕДИНСТВЕННАЯ аксиома<br/>первопричина 0 → 1 · четыре границы"}}
    ...
    FC --> CL["Коллатц 🟡<br/>collatzBoundary — закон каната"]
...
машина Тьюринга — в формализации отсутствует). Коллатц включён в репозиторий: его закон каната —
**четвёртая граница** `step00FirstCause` (`collatzBoundary`, §18); зелёный фронт — `Engine/CollatzTugOfWar`,
потребитель — `Engine/CollatzFirstCause` (главы [55](prose/55_Collatz.md)–[56](prose/56_CollatzFirstCause.md)).
...
| 55 | **Коллатц: двигатель на ограниченном топливе** | [55](prose/55_Collatz.md) | `Engine/CollatzEngine`, `Engine/CollatzFuel`, `Engine/CollatzTugOfWar` | 🟢 законы двигателя/каната + бюджет окна ⟹ спуск + «опровержение = вечный двигатель»; 🔴 вход `RopeCountingLaw` |
| 56 | **Первопричина Коллатца: заперто за двигателем** | [56](prose/56_CollatzFirstCause.md) | `Engine/CollatzFirstCause`, `Engine/CausalClosureAxiom` §18 | 🟡 ЧЕТВЁРТАЯ граница (закон каната) ⟹ `collatz_from_firstCause`; 🟢 эпистемика «проверка, а не вывод»; декрет, возможно, переплачивает |

🟢 = машинно проверено, без `sorry` (стандартные аксиомы). 🟡 = AXIOM-TAINTED (условно на
`step00FirstCause`; ровно **52** декларации — 42 в карантине, следствие
...
близнецов), **2 декларации дядического истока** (`DyadicFirstCause.dyadicOrigin_from_firstCause`,
`dyadicBlowup_is_firstCauseManifestation`, через границу `nsBoundary`), и **5 деклараций Коллатца**
(`collatzRopeBoundary`, `collatz_from_firstCause`, две растяжки, `collatz_locked_behind_engine_status`,
через **новую четвёртую границу** `collatzBoundary`) — все через принятые границы. 🔴 = открытый узел / вход.
...
«0 → 1» с четырьмя причинными границами: близнецов (`causalBoundary`), Римана (`riemannBoundary`),
Навье–Стокса (`nsBoundary`) и Коллатца (`collatzBoundary`). Аксиома заперта в карантинном модуле;
верификатор помечает каждую зависящую от неё декларацию (ровно 52: 43 в карантине + 2 геометрии гл. 49
через принятую границу близнецов + 2 дядического истока через `nsBoundary` + 5 Коллатца через
`collatzBoundary`) — утечек в зелёную линию нет. Честность
...
(2) единственная `sorryAx`-заражённая декларация — `twin_prime_conjecture`. Текущий прогон: **19550**
деклараций, sorryAx **1**, нестандартной аксиомой заражены **52** (само число растёт с контентом и
