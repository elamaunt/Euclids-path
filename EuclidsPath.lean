...
import EuclidsPath.Engine.RiemannSpectralAnchor -- якорь Гильберта–Пойи двухслойно: 🟢 SpectralRealisation ⟹ RH (дословный мост к mathlib-RH) + АУДИТ: реализация ⟺ RH даром (spectralRealisation_iff — переименование раскрыто) ⟹ содержательный слой обязан нести ОПЕРАТОР — 🔴 OperatorSpectralAnchor (самосопряжённость+спектр, mathlib-структуры); таинт не растёт
...
import EuclidsPath.Engine.PNPDataAnchor -- 🔴 data-anchor машинной модели: TM2CompositionLaw ровно там, где mathlib proof_wanted comp; TM2FrameBridge с ФИКСИРОВАННОЙ кодировкой (∃-форма машинно уличена как переименование True — tm2DecidesWithSomeEncoding_free); 🟢 модель обитаема (id); anchors_do_not_decide_pnp; таинт не растёт
...
import EuclidsPath.Engine.BSDAnalyticAnchor -- 🔴 data-anchor аналитического ранга: ContinuedLFunction (Λ + согласование с LSeriesSummable-областью + AnalyticAt в 1 — продолжения mathlib НЕ даёт) ⟹ 🟢 analyticRankOf := analyticOrderAt Λ 1 (настоящий порядок нуля) + WeakBSDContentful; контраст со свободными старыми воротами раскрыт; таинт не растёт
...
import EuclidsPath.Engine.LehmerBoundedDegree -- 🟢 Лемер НА ОГРАНИЧЕННОЙ СТЕПЕНИ доказан (lehmer_at_bounded_degree: для каждой n есть c>1 без мер в (1,c) — Норткотт-минимум окна); 🔴 UniformLehmerGap (одна c на все n) = сама гипотеза, iff-аудит раскрыт; таинт не растёт
...
import EuclidsPath.Engine.BoundedGapsAnchor -- 🔴 якорь ограниченных зазоров BoundedGaps B (Zhang 70M / Maynard–Tao 246 — НЕ формализованы, только имена) + 🟢 мосты: монотонность вверх по B, ГЛАВНЫЙ АУДИТ boundedGaps_two_iff_twins (B=2 ⟺ TwinLowers.Infinite — переименование цели раскрыто iff-ом), дизъюнкции планок 4/6 по сетке Полиньяка; спуск 246→2 НЕ утверждается (NoDescentFrom246To2Claimed); таинт не растёт
...
import EuclidsPath.Engine.OddPerfectEulerForm -- 🟢 ФЛАГМАН: теорема Эйлера — нечётное совершенное n = q^α·m², q простое, q ≡ α ≡ 1 (mod 4), q ∤ m; ступени: единственный нечётный показатель, вычеты q и α, упаковка квадрата; таинт не растёт
