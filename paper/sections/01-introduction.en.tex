...
none. The twin-prime conjecture is \emph{not} proved here. (The recent analytic advances on small gaps
between primes~\cite{gpy2009,zhang2014,maynard2015} approach the twins quantitatively, from a
different direction; ours is a structural reduction, not a bound.) What is proved, and machine-checked
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/navigator.en.pdf}
  \caption{Navigator map of the programme. One prohibition (\code{no\_infinite\_descent}); the main
  theorem \code{higherEnergyIncompatibility\_main}, green and axiom-free, pressing on the single axiom
  \code{step00FirstCause}; and every examined problem grouped by its honest status --- \emph{green}
  (axiom-free, read through the engine), \emph{axiom-tainted} (taken, not Collatz-attackable), or
  \emph{deferred}/\emph{fallen} (Collatz refuted at $n=27$). The full, interactive version of this map
  is on the companion site.}
  \label{fig:navigator}
\end{figure}
