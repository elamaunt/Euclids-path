...
загибаются сильнее фона. Мы это уже видели под другим именем — щель Янга–Миллса, цена первого
возбуждения над вакуумом, дискретная порция, без которой лестница стала бы вечным двигателем
([40](40_YangMills.md)). Масса как щель и масса как глубина колодца — одно: и то, и другое есть мера того,
насколько круто здесь загнута стрела необратимого времени. Предельная масса — само дно, сингулярность
`0`: событие `0 → 1`, которое нельзя причинить изнутри ([33](33_CausalFirstCause.md)).
...
он несёт, искажено кривизной ровно настолько, насколько искривлён его путь. Это геометрическое лицо
той самой эпистемической теоремы, что держит всю работу: *изнутри вселенной первопричину нельзя ни
вывести, ни узнать* — не оттого, что нам не хватает приборов, а оттого, что всякий сигнал идёт по
кривому, и прямой линии до истока попросту нет. Свет не лжёт — он просто не может идти прямо, а потому
не может и передать то, что было бы прямой правдой о начале.
...
обращением времени, то есть вечным двигателем (`engine_never_returns`). Горизонт событий — это граница,
за которой все нити спуска уже закручены вовнутрь. Ур-чёрная дыра — само дно, сингулярность `0`: к нему
всё стекается, а породившее его событие `0 → 1` лежит вне досягаемости изнутри. Наблюдатель в таком
...
*Мысленный эксперимент (концептуальная надстройка над реальным субстратом). Золотые точки — настоящие
twin-центры золотой спирали фрактала; **наблюдатель** (белый) сидит в центре, окружён пунктирным
**горизонтом событий**; оранжевые логарифмические спирали — **стрела времени, закрученная внутрь со
всех сторон** к наблюдателю. Всё стекается к дну, ничто не возвращается — образ необратимости
`engine_never_returns`, наложенный на реальный узор спуска.*
...
времени, сингулярность `0`. Меридианы, тянущиеся от полюса-истока к полюсу-наблюдателю, — это и есть
стрела времени, а узор фрактала близнецов обёрнут по всей сфере. Начало и настоящее не рядом и не
достижимы друг из друга по прямой: между ними — вся кривизна мира, и оттого исток остаётся за
предельной тенью.

![Мысленный эксперимент — сфера времени: наблюдатель и старт времени на антиподах](../tools/fractal/out/8_time_sphere.png)

*Мысленный эксперимент (концептуальная надстройка). **Сфера времени**: наблюдатель (белый) — на одном
полюсе, **старт времени · сингулярность `0`** — в антиподе на обратной стороне; оранжевые меридианы —
**стрела времени** от истока к наблюдателю; золотые точки — реальные twin-центры, разложенные
сферической спиралью Фибоначчи. Исток и «сейчас» максимально далеки: между ними — вся кривизна, и
прямого луча от одного к другому нет.*
