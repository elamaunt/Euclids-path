...
four-corner плюс нижняя оценка carrier — не выводится distribution-free из имеющихся идей. Естественно
спросить: нет ли у остановки двигателя **алгебраической** цены, которую можно предъявить без
...
> **Определение (канальный класс).** При заданных `a, ε, σ` и простом `p` источник `m` называется
...
**Теорема (`channel_residue`).** Если `p > 3`, `6m + σ = a(6n + ε)` и `p ∣ 6n − ε`, то
...
> **Определение (сдвиг θ).** Полагаем `θ := σε`. Проход по `q` называется *безналоговым*, если
> дополнительный запрет по `q` на стороне `6m − σ` совпадает с ранее исключённым классом, то есть не
> отнимает новой ёмкости.

**Теорема (`no_tax_iff_shifted`).** Безналоговость по `q` равносильна делимости сдвига:
...
> **Определение (примориал по `G`).** Для конечного множества попарно взаимно простых `q`
> обозначим `P := \prod_{q \in G} q`. Это примориал (произведение) малых простых, по которым проход
> безналоговый.

**Теорема (`primorial_dvd_shift`).** Если `(G).Pairwise Nat.Coprime` и `∀ q ∈ G, q ∣ a − θ`, то
...
**Теорема (`shifted_primorial_bound`).** Если `P ∣ a − θ` и `a ≠ θ`, то
...
активным делителем: `P_{<p} \le |a - θ|`. Как только примориал перерастает активный делитель,
нетривиального безналогового прохода просто **нет**.

> **Определение (порог `Y_A`).** Пусть `Z` — верхняя граница активного делителя на масштабе `A`
...
**Теорема (`late_boundary_not_free`).** Если `P ∣ a − θ`, `|a − θ| ≤ Z` и `Z < P`, то `a = θ`.

Это контрапозиция закона дефекта: предположив `a ≠ θ`, из `shifted_primorial_bound` получаем
...
Нет такого `D = D(A)`, при котором **оба** бюджета одновременно `o(|S_0|)` без обращения к
распределению.

> **Гипотеза (единственный открытый вход маршрута оплаты).** Существует функция `D(A)`, при которой
...
из descent клинового центра, а не взято случайно. В главе 18 мы совершаем именно этот сдвиг стратегии:
через rank descent все product-state дефекты сводятся к rank-1, а rank-1 — к единственной, **по
построению не счётной** лемме SNOL о терминальном сдвинутом соседе `p \mid a - 2\varepsilon`. Там, где
маршрут оплаты тайно звал распределение, SNOL его **запрещает** и требует евклидову родословную `a`.
