...
- ⚠️ **Единственный `axiom` репозитория (карантин):** `Engine/CausalClosureAxiom.lean` объявляет
  ...
  через аксиому не замыкается. Карантин машинно отслеживается: узловой верификатор репортит каждую
  AXIOM-TAINTED декларацию (ровно 5, все в карантинном модуле; утечки в основную линию нет).
