...
import EuclidsPath.Engine.BealFront -- СОСЕД (глава 54): тень Бил/Ферма–Каталан — чистый Ферма-спуск = движок. 🟢 РЕАЛЬНЫЙ Polynomial.flt_catalan (ДОКАЗАН, спуск по char p) + fermatLastTheoremFour/Three (спуск) + descent-без-двигателя (реюз EPMI/UniversalEngine). 🔴 BealConjecture, FermatCatalanConjecture открыты (Дармон–Гранвиль: лишь фикс. показатели). НЕ доказательство; таинт не растёт
