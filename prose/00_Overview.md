...
> `Engine/CausalClosureAxiom.lean`, ровно 45 таких деклараций — 43 в карантине плюс 2 декларации
> геометрии гл. 49 (пересечение прямых из той же границы близнецов), утечек нет — верификатор
...
зелёное, без всякой аксиомы, и держится оно на простой мысли: *узнать причину изнутри стоит энергии,
которой в замкнутой системе нет.*
...
близнеца, только если *весь* его конечный класс — близнецы, а близнец в смешанном классе для него
невидим; значит, и бесконечность близнецов изнутри неудостоверима. Две стены — причинная и
познавательная — оказываются **одной природы** (`two_walls_one_nature`): изнутри конечной системы не
видно ни её первопричины, ни бесконечности близнецов. И тогда несущая, пятая грань замыкает круг:
сама эта несовместимость (двигателей нет = причину не узнать) вместе с принятой причинной границей
**влечёт бесконечность близнецов**.

Прочитать всё вместе стоит энергетически: **«знать изнутри» стоит вечного двигателя, которого нет; а
бесконечность близнецов — это внешнее знание, оплаченное первопричиной.** Следствие
Короче всего теорема звучит так: **знать, что близнецы бесконечны, нельзя; но если незнание
первопричины принять за истину — они бесконечны, и это строго.** Следствие
`higherEnergyIncompatibility_twins` (`TwinLowers.Infinite`) — 🟡: это пятая грань, инстанциированная
...
И у всей этой конструкции есть строгое космологическое прочтение (теоремы строги, космология — их
перевод; полный пошаговый разбор — в [главе 33](33_CausalFirstCause.md)). Числовая прямая и
невозможность двигателя вместе кодируют пространство и время: строгий порядок обхода — это пространство
с дном-сингулярностью в `0`, а необратимая **стрела времени доказана строго**
(`engine_never_returns` — высота строго антимонотонна, назад хода нет). Первопричина — миг, когда
двигатель появляется из сингулярности `0` и трогается вперёд не сворачивая; дать её изнутри нельзя,
ведь самозапуск был бы вечным двигателем (`no_internalisedOriginEvent`), — потому она принимается
извне, единственной аксиомой. Оттого внутри системы бесконечность близнецов нельзя ни доказать, ни
опровергнуть: оба действия означали бы построить вечный двигатель. А башни вселенных под первопричиной
нет — регресс причин вполне-фундирован (`no_rankedMetaFractalBranch`), вселенная одна.
...
ровно 45 AXIOM-TAINTED деклараций (42 в карантинном модуле + задокументированное следствие
`higherEnergyIncompatibility_twins` + 2 декларации геометрии главы 49, обе через УЖЕ ПРИНЯТУЮ
границу близнецов, без нового поля), верификатор репортит каждую. Третьей (P/NP) границы НЕТ —
