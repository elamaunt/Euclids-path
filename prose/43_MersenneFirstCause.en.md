...
**Definition (`MersenneTwinAbsenceAbove P`).** *Absence of Mersenne twins above `P`: every
pair `(2^p − 3, 2^p − 1)` of primes sits with its lower side no higher than `P`.*
...
(`mersenneTwinCentersUnbounded_iff_no_absence` 🟢). About the witness's domain we also know a lower bound:

**Theorem** (`mersenneAbsenceBound_ge_29`, 🟢). *Every absence bound is at least 29.*
...
`m → 4m + 1`; it is strictly monotone and injective (`mersenneCenterChain_strictMono` 🟢). Outwardly it is
a twin of the pentadic chain `c → 5c + 1`, which in [24](24_BoundaryDecomp.md) forged an infinite family of genealogies and
refuted the node's minor branch. But the resemblance is deceptive, and the difference is arithmetic:

**Theorem** (`isEmpty_properCenterPeel_five_one`, 🟢 — even without Classical.choice). *The first step
...
**Definition (`MersenneManifestationLaw`).** *For every threshold `P` with an absence witness:
...
**Theorem** (`mersenneRefutation_carries_engine`, 🟢 — readable form). *An absence witness +
...
**Theorem** (`mersenneTwinsUnbounded_of_noEngine_and_boundary_and_manifestation`, 🟢 — essence).
...
**Theorem** (`mersenneManifestationLaw_iff_unbounded_of_boundary`, 🟢 — the chief audit M7). *Under
