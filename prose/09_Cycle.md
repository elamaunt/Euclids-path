...
В [08. Ограниченный цикл](08_BK.md) мы извлекли из pigeonhole (пижонхол — принцип ящиков, см. [словарь](GLOSSARY.md)) на симметрических суммах ограниченный аддитивный цикл: при достаточной массе повторов делители не могут располагаться произвольно, они замыкаются в конечную аддитивную структуру.
...
Напомним конфигурацию, унаследованную от carrier'а [02. Носитель двойки](02_Carrier.md) и descent'а [04. Спуск](04_Descent.md).

Центры двигателя (двигатель — запрещённый бесконечный спуск, см. [словарь](GLOSSARY.md)) вдоль одной аффинной линии образуют арифметическую прогрессию: значение, «сканируемое» простым делителем в точке с координатой $n$, имеет вид
$$
V(n) = N_0 + q\cdot n,
$$
...
$$
Двойка евклидова уравнения «путешествует» между сторонами: делитель $p$, чтобы встать сразу на обе стороны звёздного прямоугольника, обязан согласовать межстороннюю разность $\beta-\alpha$ с $2/a \bmod p$. Это и есть перенос топлива $+2$ поперёк сторон (детально механизм переноса разбирается в [11. Блочное ядро](11_TwoTransport.md) и синтезируется как «первый закон — сохранение двойки»).
