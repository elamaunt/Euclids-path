...
А значит, декретировать закон квантования для модели значило бы декретировать её щель дословно — переименовать цель, а не добавить причину. Трилемма — обязательная трёхчастная проверка кандидата в границы (см. [словарь](GLOSSARY.md)) — подтверждает это со всех сторон:

- универсальная форма закона *опровержима* — кованая (то есть машинно построенная как контрпример) безмассовая модель `{0} ∪ {2⁻ⁿ}` несёт явную лестницу `cookedLadder`, и декрет взорвал бы карантин;
...
Единственное 🟡-содержание ветви — то, что декрет утверждает *сам*, на своём масштабе. При `A ≤ 4` поставка отклонений — то, чем отклонение «расплачивается» (см. [словарь](GLOSSARY.md)), — реальна (`smallScale_deviationSupply` 🟢, пятиадическая цепь — объект манифестации, не пустая форма).
...
Мы намеренно не добавили фальшивого 🟡 «масс-гэп ранго-мира»: натуральная щель ранго-мира тривиально зелёная. Растяжка — намеренный детектор взрыва (см. [словарь](GLOSSARY.md)) — `quarantine_inconsistent_if_supply_at_every_scale` 🟡 держит точку взрыва видимой.
...
У стены, о которую разбиваются внутренние решения Коллатца и P/NP ([глава 56](56_CollatzFirstCause.md)), есть и спектральный склон — эпистемический комплемент ветви собран в `Engine/YangMillsEpistemic.lean`.
...
И третья створка развилки `ym_no_internal_decision_without_engine` 🟢 — фирменная особенность именно этой ветви. Решает вопрос только per-model закон, но `quantizationLaw_iff_massGap` 🟢 зелёно и без всякой границы: закон решает вопрос ровно потому, что он *и есть* вопрос.
