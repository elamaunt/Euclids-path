...
\begin{definition}[twin centre and lower twins]\label{def:istwincenter}
For $m\in\N$,
\[
...
exactly \code{TwinLowers.Infinite}. \leantag{IsTwinCenter}, \leantag{TwinLowers}
\end{definition}
...
\begin{theorem}[a survivor of an uncovered set]\label{thm:survivor_of_not_covered}
\green{}\ For finite $\Omega,\mathrm{bad}\subseteq\N$,
\begin{equation}\label{eq:survivor}
  |\mathrm{bad}|<|\Omega|\;\Longrightarrow\;\exists\,m\in\Omega,\ m\notin\mathrm{bad}.
\end{equation}
\leantag{survivor\_of\_not\_covered}
\end{theorem}
...
\begin{theorem}[primality by a root sieve]\label{thm:prime_of_no_small_prime_factor}
\green{}\ Let $n\ge 2$ and suppose $\forall p\ \text{prime},\ p^2\le n\Rightarrow p\nmid n$. Then $n$
is prime. \leantag{prime\_of\_no\_small\_prime\_factor}
\end{theorem}
...
\begin{theorem}[a root-sieved centre is a twin centre]\label{thm:isTwinCenter_of_root_sieve}
\green{}\ If $6m\pm1\ge 2$ and no prime up to the root of either side divides that side, then
$\mathrm{IsTwinCenter}(m)$. \leantag{isTwinCenter\_of\_root\_sieve}
\end{theorem}
...
substantive difficulty lies not here but in \emph{producing} a survivor sieved to the root.

The third link is order-theoretic.

\begin{theorem}[unbounded centres give infinitely many twins]\label{thm:infinite_of_unbounded_centers}
\green{}\ If for every $N$ there is a centre $m>N$ with $\mathrm{IsTwinCenter}(m)$, then
\code{TwinLowers.Infinite}. \leantag{infinite\_of\_unbounded\_centers}
\end{theorem}
...
\begin{theorem}[a twin centre from a block]\label{thm:twin_center_of_block}
\green{}\ Let $\mathit{carrier},\mathit{bad}\subseteq\N$ be finite with
...
\end{equation}
Then $\exists m,\ N<m\ \wedge\ \mathrm{IsTwinCenter}(m)$. \leantag{twin\_center\_of\_block}
\end{theorem}
...
lines of \code{Finset} combinatorics, no appeal to prime density.

\begin{theorem}[the block reduction of the twin conjecture]\label{thm:twin_prime_conjecture_of_blocks}
\green{}\ If for every $N$ there exist finite $\mathit{carrier},\mathit{bad}$ satisfying
\cref{eq:block}, then \code{TwinLowers.Infinite}. \leantag{twin\_prime\_conjecture\_of\_blocks}
\end{theorem}
...
centre above $N$. The implication is non-circular: it nowhere uses the conjecture, nor hides
...
\begin{theorem}[twins from the four-corner input]\label{thm:twin_primes_of_four_corner}
...
\end{equation}
\leantag{twin\_primes\_of\_four\_corner}
\end{theorem}
...
\begin{theorem}[finite twins contradict the four-corner]\label{thm:twin_finite_contradiction}
\green{}\ If $\neg\,\code{TwinLowers.Infinite}$ and \code{FourCornerInput} $H$, then \code{False}.
\leantag{twin\_finite\_contradiction}
\end{theorem}
...
Logically contraposition adds nothing, but it makes tangible what breaks when the thesis is
...
\begin{theorem}[the shifted-primorial bound]\label{thm:shifted_primorial_bound}
\green{}\ If $P\mid a-\theta$ and $a\ne\theta$, then $P\le|a-\theta|$.
\leantag{shifted\_primorial\_bound}
\end{theorem}

\emph{Proof idea.} From $a\ne\theta$ we have $a-\theta\ne0$, so $|a-\theta|>0$ and
\code{Int.le\_of\_dvd} gives $P\le|a-\theta|$. This is the \emph{defect law}: once the
...
\begin{theorem}[twins from the SNOL input]\label{thm:twin_primes_of_SNOL}
\green{}\ If \code{SNOLInput} holds --- at every scale $N$ a carrier above $N$ whose bad centres
(carriers of the terminal shifted-neighbour obstruction $p\mid a-2\varepsilon$, $p\le A$) are strictly
fewer, every survivor a twin --- then \code{TwinLowers.Infinite}. \leantag{twin\_primes\_of\_SNOL}
\end{theorem}
...
contraposition. Conditional on the \red{} node \code{SNOLInput}. The neighbour audit shows
...
\begin{theorem}[old-peel drops the height]\label{thm:old_peel_height_drop}
\green{}\ If $6n-\varepsilon=p\,(6t+\delta)$ with $p\ge5$, $\varepsilon,\delta\in\{\pm1\}$, $t\ge1$,
$n\ge2$, then $t<n$. \leantag{old\_peel\_height\_drop}
\end{theorem}

\emph{Proof idea.} From $6t+\delta=(6n-\varepsilon)/p\le(6n+1)/5$ and $n\ge2$ we get $t<n$
(\code{nlinarith}). Numerically the descent is sharper still --- $t<n/5$ on $3000/3000$ real
rank-1 catches. Every catch is a step downward; the flow cannot ride down forever.

\begin{theorem}[no infinite old-peel chain]\label{thm:no_infinite_old_peel}
\green{}\ For any $z:\N\to\N$ with \code{StrictAnti}\,$z$, \code{False}.
\leantag{no\_infinite\_old\_peel}
\end{theorem}

\emph{Proof idea.} This is \code{no\_infinite\_engine\_descent} applied to the old-peel height chain:
an infinite strictly descending chain in $\N$ does not exist. The final SNOL node thus closes
...
\begin{definition}[old-peel ledger]\label{def:oldpeelledger}
...
\begin{theorem}[twins from a family of ledgers]\label{thm:twin_primes_of_nopsl}
\green{}\ Given a family of ledgers $L(N)$ with starts and a centre map, and the condition
...
\leantag{twin\_primes\_of\_nopsl}
\end{theorem}
...
twin centre above $N$, and \cref{thm:infinite_of_unbounded_centers} concludes. The full
...
\begin{theorem}[the regeneration dichotomy]\label{thm:regeneration_dichotomy}
\green{}\ For every centre $t$ and threshold $A$, exactly one holds:
\[
\begin{gathered}
\mathrm{Twin}(t)\ \lor\ \bigl(\neg\mathrm{OldFree}(A,t)\wedge\exists q\ \text{prime},\,5\le q\le A,\,q\mid 6t\pm1\bigr)\\
\lor\ \bigl(\mathrm{OldFree}(A,t)\wedge(\neg(6t-1)\ \text{prime}\ \lor\ \neg(6t+1)\ \text{prime})\bigr).
\end{gathered}
\]
\leantag{regeneration\_dichotomy}
\end{theorem}
...
a divisor $>A$ (\code{composite\_oldfree\_has\_big\_divisor}), an active-descent edge.
...
\begin{theorem}[a clean sink below the square is a twin]\label{thm:sink_is_twin}
\green{}\ If both sides satisfy $2\le 6m\pm1<A^2$ and no prime $q\le A$ divides either side, then
$\mathrm{TwinCenterZ}\,m$. \leantag{sink\_is\_twin}
\end{theorem}
...
\code{oldfree\_below\_sq\_prime}, one per side. Cleanliness excludes small divisors, the
...
\begin{theorem}[the clean-centre outcome]\label{thm:clean_center_outcome}
\green{}\ For every clean centre $m$, exactly one holds:
\[
...
\]
\leantag{clean\_center\_outcome}
\end{theorem}

\emph{Proof idea.} Nested case analysis on twin-ness and on the existence of an active edge, closed by
\code{active\_edge\_clean\_or\_boundary} (excluded middle on $\mathrm{Clean}\,A\,n$). The
...
\begin{theorem}[a global absorber forces an engine]\label{thm:global_absorber_forces_engine}
\green{}\ Let $S$ be infinite, $\mathrm{Codom}$ finite, and $\mathrm{key}:\alpha\to\mathrm{Codom}$ a
start's passport. Given the pump
...
\end{equation}
then \code{Engine}. \leantag{global\_absorber\_forces\_engine}
\end{theorem}
...
\begin{theorem}[a regenerating graph reaches a twin]\label{thm:reaches_twin}
\green{}\ If $G$ regenerates (every non-twin state has a descending successor), then
$\forall s,\ \exists t,\ \mathrm{Twin}\,t$. \leantag{reaches\_twin}
\end{theorem}

\emph{Proof idea.} Strong induction on the height of the start: a non-twin state steps down to a
strictly smaller height, and one cannot descend forever in $\N$. The cycle branch and its
...
\begin{theorem}[the cofactor is a smaller centre]\label{thm:cofactor_is_center}
\green{}\ For $t\ge1$, $\eta\in\{\pm1\}$, a prime $q\ge5$ coprime to $6$ with $q\mid 6t+\eta$, there
exist $t',\eta'$ with $\eta'\in\{\pm1\}$, $6t'+\eta'=(6t+\eta)/q$ and $0\le t'<t$.
\leantag{cofactor\_is\_center}
\end{theorem}

\emph{Proof idea.} The cofactor $c=(6t+\eta)/q$ is forced coprime to $6$ (both $6t+\eta$ and $q$ are,
so $c\bmod6\in\{1,5\}$), positive, and $<t$ since $5c\le 6t+1$. Checked at $100\%$ on
...
\begin{theorem}[no product-Hall collision]\label{thm:no_productHall}
\green{}\ If at role $k$ both factors are $<P_A$ and congruent mod $P_A$, yet distinct, then
\code{False}. \leantag{no\_productHall}
\end{theorem}

\emph{Proof idea.} For $a<P_A$ we have $\code{ZMod.val}(a\bmod P_A)=a$, so equal residues force equal
factors, against the mismatch. Removing the tail from the induction object removes the
...
\begin{theorem}[the core descent step]\label{thm:core_step_proved}
\green{}\ Under the separating scale, $\forall r,\ 2\le r\Rightarrow\code{CoreCollision}\,X_A\,P_A\,r
\Rightarrow\code{CoreCollision}\,X_A\,P_A\,(r-1)$. \leantag{core\_step\_proved}
\end{theorem}
...
--- a ProductHall, impossible by \cref{thm:no_productHall}. The base rank~$1$ is closed by the
...
\begin{theorem}[an engine from the product core]\label{thm:product_core_engine_of_carrier}
\green{}\ Under the separating scale, $1\le r\le4$, and an infinite $S$ of ambient-legal
\code{RankNode}s with $\mathrm{node}$ injective on $S$, we obtain \code{Engine}.
\leantag{product\_core\_engine\_of\_carrier}
\end{theorem}
...
(\code{coreCollision\_of\_infinite}). A \emph{conditional} theorem: everything between the
...
\begin{theorem}[a legal node from a composite side]\label{thm:mkNode_of_composite}
\green{}\ Let $1\le A$, $1<N$, $N$ composite, $N\le 6X_A+1$, $6X_A+1<A^5$, and every prime divisor of
$N$ greater than $A$. Then for any sign there is a rank $r$ with $2\le r\le4$ and $X:\code{RankNode}\,r$
with $\code{AmbientLegal}\,X_A\,X.\mathrm{factors}$ and $\prod(\mathrm{ofFn}\,X.\mathrm{factors})=N$.
\leantag{mkNode\_of\_composite}
\end{theorem}
...
\code{AmbientLegal}. The magic number $4$ is an arithmetic consequence of the sieve scale, not
a convention. Joining this node factory to the proven infinitude of the carrier:

\begin{theorem}[the clean centres are infinite]\label{thm:cleanCenters_infinite}
\green{}\ For every $A$, the set $\mathrm{CleanCenters}(A)=\{m\mid\mathrm{CleanZ}\,A\,m\}$ is infinite.
\leantag{cleanCenters\_infinite}
\end{theorem}

\emph{Proof idea.} By \code{Set.infinite\_of\_not\_bddAbove}: for any $N$, \code{carrier\_nonempty\_above}
produces a clean $m>N$ (a multiple of the old primorial). No density is needed.

\begin{theorem}[an engine from carrier and factorisation]\label{thm:engine_of_carrier_and_factorize}
\green{}\ Under the separating scale, an infinite carrier $C$, and a rank map \code{rankOf} and node map
\code{mkNode} injective and \code{AmbientLegal} on each rank, \code{Engine} holds.
\leantag{engine\_of\_carrier\_and\_factorize}
\end{theorem}
...
\cref{thm:product_core_engine_of_carrier}. The infinitude of the carrier, the fibre choice,
...
\begin{conjecture}[the single open node]\label{conj:globaloldabsorption}
...
yielding distinct nodes. \leantag{GlobalOldAbsorption}
\end{conjecture}
