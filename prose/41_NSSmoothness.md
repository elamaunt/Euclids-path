...
Но кованый — то есть машинно построенный как контрпример (см. [словарь](GLOSSARY.md)) —
...
Это тот же урок, что дала лестница Янга–Миллса: суррогат говорит именно о *равномерном*
...
энергобаланс честной границей декрета, гейченной формой закона. Расширенная трилемма — обязательная
тройная проверка кандидата в границы: опровержимость, вакуумность, переименование цели
(см. [словарь](GLOSSARY.md)) — дала нетривиальную и поучительную картину.
...
именованное условие-вход, которого требует честная формулировка (см. [словарь](GLOSSARY.md)), —
...
суррогат, не `C^∞`), тождество энергии (`energyIdentity_from_firstCause`) и растяжка — намеренный
детектор взрыва на случай опровержения закона (см. [словарь](GLOSSARY.md)) —
...
**Вывод.** Суррогату гладкости хватило бы даже *неравенства* (`noSingularCascade_of_twoTimeInequality` 🟢), а декрет утверждает более сильное *равенство* `dE/dt = −D`. Обратная импликация неизвестна — то есть **декрет, возможно, переплачивает** за свою цель. Это тот же паттерн, что мы честно отметим у Коллатца: цена может превышать стоимость.
