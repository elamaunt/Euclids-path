...
(вакуумность — когда кандидат-закон выполняется даром и декрет был бы пуст; см. [словарь](GLOSSARY.md)).
...
> 🟢 **Теорема 34.3** (`isTwinCenter_mersenneCenter_iff`). Для нечётного $p \ge 2$ центр $m_p$ —
> twin-центр $\iff$ оба числа $2^p-3$ и $2^p-1$ просты.

Такие «Мерсенн-близнецы» существуют:

> 🟢 **Теорема 34.4** (`mersenne_twin_instances`). Центры $m_3$ и $m_5$ — twin-центры: $p=3$ даёт
> центр $1$ с парой $(5,7)$, а $p=5$ — центр $5$ с парой $(29,31)$.

Дальше по $p$ совпадения быстро иссякают — и
...
> 🟢 **Теорема 34.6** (`twinLowersInfinite_of_mersenneTwins`). `MersenneTwinCentersUnbounded` ⟹
> `TwinLowers.Infinite`.
...
`MersennePaymentConflict.lean` переносит на Мерсенн бухгалтерию из [17. Payment ledger](17_PaymentLedger.md)
(леджер — бухгалтерия оплачиваемых потоков; см. [словарь](GLOSSARY.md)). Центры
...
> 🟢 **Теорема 34.7** (`pressure_iff_supply_for_everythingPrimeLedger`). При обитаемом входе
> обычной twin-поддержки: кофинальное давление $\iff$ бесконечность Мерсенн-близнецов
> (`InfinitelyManyMersenneTwinCenters`) — то есть давление ⟺ переименованный **вывод**.

Аналогично для канонической peel-системы `canonicalPeelSystem` («hit = уже twin», payment law —
🟢 `canonical_paymentLaw` дефинициально):

> 🟢 **Теорема 34.8** (`canonical_coverage_iff`). При обитаемом входе twin-поддержки: coverage
> $\iff$ бесконечность Мерсенн-близнецов — тот же вывод.

**Вывод.** На произвольном леджере входы пусты: предположить давление — значит предположить
заключение — знакомое по трилеммам «переименование цели» (см. [словарь](GLOSSARY.md)).
Расщепления имеют содержание только для *настоящего* Step00-леджера генеалогий, где
...
из [17](17_PaymentLedger.md)–[24](24_BoundaryDecomp.md), hit — реальное попадание генеалогии в repunit-центр (base-4 peel как
...
делителей (как $6 = 1+2+3$ или $28 = 1+2+4+7+14$).

Спустя две тысячи лет Эйлер замкнул обратное: всякое
...
по-прежнему открыто, как и во времена Евклида.

Наша ветка честно останавливается ровно там, где
...
После этой главы ветка перестала быть только «боковой». В [главе 43](43_MersenneFirstCause.md)
её утверждение об отсутствии проведено через манифестационную архитектуру Римана
(манифестация, непредъявимый свидетель, кованое опровержение — см. [словарь](GLOSSARY.md)).
...
Трилемма четвёртой границы пройдена, но поле
**намеренно отложено**: при границе закон ⟺ неограниченности, а эвристика впервые направлена
против — см. честную цену со знаком в [43](43_MersenneFirstCause.md). Вакуумность №3 при этом не ретушируется: форвард-серия
остаётся необитаемой, новый фронт не разделяет с ней ни одного определения.
