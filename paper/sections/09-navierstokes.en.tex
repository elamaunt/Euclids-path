...
\begin{definition}[Navier--Stokes solution]\label{def:IsNSSolution}
...
\begin{theorem}[the equation is inhabited]\label{thm:zero_is_NSSolution}
\green{}\ $\code{IsNSSolution}\ \nu\ 0\ 0\ 0$ for every $\nu \in \R$: the zero field with zero
pressure and no force is a solution at any viscosity. \leantag{zero\_is\_NSSolution}
\end{theorem}
...
\begin{theorem}[kinetic energy of a non-integrable field]\label{thm:kineticEnergy_of_not_integrable}
\green{}\ If $\|u\|^2$ is not integrable (the predicate \code{FiniteKineticEnergy} fails), then
$\code{kineticEnergy}\ u = 0$. \leantag{kineticEnergy\_of\_not\_integrable}
\end{theorem}
...
\begin{theorem}[two-time inequality from the pointwise balance]\label{thm:twoTimeEnergyInequality_of_energyBalance}
\green{}\ If the pointwise balance $\code{EnergyBalanceLaw}\ \nu\ u$ holds and $s \mapsto D(u(s))$ is
interval-integrable on every $[t_1,t_2]$, then $\code{TwoTimeEnergyInequality}\ \nu\ u$ holds.
\leantag{twoTimeEnergyInequality\_of\_energyBalance}
\end{theorem}
...
\begin{theorem}[no infinite dissipative cascade]\label{thm:ns_no_infinite_dissipative_cascade}
\green{}\ (conditional on the inequality input) Let $\delta > 0$ and suppose
$\code{TwoTimeEnergyInequality}\ \nu\ u$ holds. Then there is no sequence of times $(t_k)_{k\in\N}$
with $\code{DissipativeStage}\ \nu\ u\ \delta\ t_k\ t_{k+1}$ for all $k$.
\leantag{ns\_no\_infinite\_dissipative\_cascade}
\end{theorem}
...
\begin{theorem}[positive real work need not be well-founded]\label{thm:real_positive_work_not_wellfounded}
\green{}\ There exists $a : \N \to \R$ with $a_{n+1} < a_n$, $a_n > 0$ and $a_n - a_{n+1} > 0$ for
all $n$: an $\R$-sequence with strictly positive work at every step yet a finite total descent.
\leantag{real\_positive\_work\_not\_wellfounded}
\end{theorem}
...
\begin{theorem}[the exact energy identity]\label{thm:energy_identity_of_energyBalance}
\green{}\ Under $\code{EnergyBalanceLaw}\ \nu\ u$ and interval-integrability of the dissipation, for
all $t_1,t_2$ one has
\begin{equation}\label{eq:ns-energy-identity}
E(u(t_2)) = E(u(t_1)) - \int_{t_1}^{t_2} D(u(s))\,ds,
\end{equation}
as an equality; the two-time inequality of
\cref{thm:twoTimeEnergyInequality_of_energyBalance} is a coarsening of it.
\leantag{energy\_identity\_of\_energyBalance}
\end{theorem}
...
\begin{theorem}[no singular cascade from the balance]\label{thm:noSingularCascade_of_energyBalance}
\green{}\ The pointwise energy balance $dE/dt = -D$ together with integrability implies that no
rupture points of cascade type exist: $\code{NoSingularCascade}\ \nu\ u$.
\leantag{noSingularCascade\_of\_energyBalance}
\end{theorem}
...
\begin{theorem}[no singular cascade, assembled on the box]\label{thm:noSingularCascade_of_r3Assembly}
\green{}\ For a box-supported $C^2$ solution with force $f = 0$, cascade smoothness
$\code{NoSingularCascade}$ is derived in green, without the first-cause decree \code{nsBoundary}.
\leantag{noSingularCascade\_of\_r3Assembly}
\end{theorem}
...
\begin{theorem}[Clay-(A) from transfer and vorticity control]\label{thm:clayA_of_regularityTransfer_and_vorticityControl}
\green{}\ The known transfer theory --- Kato's local strong existence \cite{kato1984} together with
the Beale--Kato--Majda continuation criterion \cite{bkm1984} --- and the single criterion
\code{GlobalVorticityControl} imply Clay-(A).
\leantag{clayA\_of\_regularityTransfer\_and\_vorticityControl}
\end{theorem}
...
\begin{theorem}[the budget is strictly weaker than vorticity control]\label{thm:greenBudget_strictly_weaker_than_vorticityControl}
\green{}\ The green budget machine is strictly weaker than \code{GlobalVorticityControl}: it does not
imply the criterion. \leantag{greenBudget\_strictly\_weaker\_than\_vorticityControl}
\end{theorem}
...
before; the novelty is formalisational, not mathematical --- nothing here is solved.) The strategy
--- forcing a finite-time blow-up in a carefully modified fluid equation --- is in the spirit of
Tao's averaged three-dimensional {Navier--Stokes} blow-up~\cite{tao2016}. Where the
...
\begin{theorem}[superlinear drive forces blow-up]\label{thm:superlinear_blowup_sq}
\green{}\ No global positive $C^1$ function can satisfy $y'(t) \ge C\,y(t)^2$ with $C > 0$: the
assumption of a global solution yields \code{False}. \leantag{superlinear\_blowup\_sq}
\end{theorem}
...
\begin{theorem}[sublinear drive forces collapse]\label{thm:sublinear_collapse_extinction}
\green{}\ Dually: no global positive $C^1$ function can satisfy $r'(t) \le -C\,r(t)^{\alpha}$ with
$C>0$ and $0 \le \alpha < 1$; the assumption of a global solution yields \code{False}. Collapse to
$r=0$ occurs in finite time, no later than $r(0)^{1-\alpha}/((1-\alpha)C)$.
\leantag{sublinear\_collapse\_extinction}
\end{theorem}

\begin{proof}
Put $v(t) := r(t)^{1-\alpha} + (1-\alpha)C\,t$. Then
$v'(t) = (1-\alpha)r^{-\alpha}r' + (1-\alpha)C \le 0$ (from $r'\le -C\,r^{\alpha}$ and
$r^{\alpha}r^{-\alpha}=1$), so $v$ is non-increasing, whence $r^{1-\alpha} = v - (1-\alpha)C\,t$ must
go negative in finite time, contradicting $r>0$.
\end{proof}

\emph{Why this is the exact dual.} Blow-up and collapse are one drive $u' = \pm C\,u^{\beta}$ read on
opposite sides of the critical exponent $\beta=1$: super-linear ($\beta>1$) runs to $\infty$,
sub-linear ($\beta<1$) to $0$, and at $\beta=1$ there is no finite-time singularity. The same
substitution $w := u^{1-\beta}$ linearises both, giving the same antitone auxiliary and the same
mean-value step. The novelty is formalisational, not mathematical: the extinction fact is classical
and the proof is the mirror of the blow-up above. Deriving the drive $r'\le -C\,r^{\alpha}$ from a
physical self-gravity law (Euler--Poisson, Larson--Penston self-similar collapse) is a named
analytic input beyond \code{mathlib}, \emph{not} done here; the ``collapse / supernova / Big-Bang''
readings are metaphor, not physics. A dual appendix model, not a millennium mask.

\begin{theorem}[the dyadic model blows up]\label{thm:dyadic_blowup}
\green{}\ The Katz--Pavlovi\'c model \code{DyadicSolution}, with equations
$a_n' = \lambda^n a_{n-1}^2 - \lambda^{n+1} a_n a_{n+1} - d_n a_n$, is globally empty: blow-up is
inevitable. \leantag{dyadic\_blowup}
\end{theorem}
...
\begin{theorem}[the front drive from the invariant]\label{thm:frontDrive_of_invariant}
\green{}\ Suppose a Katz--Pavlovi\'c solution has one front shell pinched from below by its two
neighbours --- the invariant \code{FrontDomination}: $\rho\, a_{J+1} \le a_J$,
$a_{J+2} \le \kappa\, a_{J+1}$, $m \le a_{J+1}$. Then the drive $C\,y^2 \le y'$ is derived directly
from the $\lambda^n$-couplings, with $C = \lambda^{J+1}\rho^2 - \lambda^{J+2}\kappa - d_{J+1}/m$.
\leantag{frontDrive\_of\_invariant}
\end{theorem}
...
inside the couplings. Precisely, the origin obeys the pure unforced coupling \emph{if and only if} the
pump vanishes (\green{}\ \code{ssOrigin\_selfCaused\_iff\_noPump}); since the forcing is strictly
positive, self-ignition is impossible --- and this ties the origin to an \emph{external} pump only,
saying nothing about that pump being the first cause. The self-similar solution needs an external
pumping term, and that supply is drawn from the first cause.

\begin{theorem}[the dyadic origin is a first-cause manifestation]\label{thm:dyadicBlowup_is_firstCauseManifestation}
\yellow{}~(\textsc{axiom-tainted})\ The supply at scale $n = 0$ is drawn from the boundary
\code{nsBoundary} of the axiom \axiom{}: the dyadic blow-up joins the first-cause manifestations
through its origin. \leantag{dyadicBlowup\_is\_firstCauseManifestation}
\end{theorem}
...
\paragraph{Not the Big Bang, but one of its faces.}
It is tempting to overread the origin cosmologically; we do not. In the cosmological translation the
Big Bang is the first cause itself, the event $0\to1$; Navier--Stokes is only one of its three
boundaries (\code{step00FirstCause\_iff\_causalClosure}: the first cause is equivalent to the
conjunction of the twin node, the Riemann law, and the NS gate). The fluid blow-up is not the
beginning of the world --- its origin $n=0$, unable to start itself, only borrows a spark from that
same single beginning: one of three faces of one singularity, not the singularity itself. As
everywhere in the programme, the cosmology is a translation of the theorems, not a claim of physics.
