...
```
Здесь левая часть — $N_{00}\cdot N_{33}$, правая — $N_{03}\cdot N_{30}$ буквально в форме модельных счётов. Доказательство — прямое следствие `four_corner_binom`: умножаем неравенство $20\binom{n}{6}\le\binom{n}{3}^2$ на $w^6\ge0$ (снова `gcongr`) и перегруппировываем множители (`ring`).
