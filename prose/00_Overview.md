...
ровно 47 AXIOM-TAINTED деклараций (42 в карантинном модуле + задокументированное следствие
`higherEnergyIncompatibility_twins` + 2 декларации геометрии главы 49 через принятую границу близнецов
+ 2 декларации дядического истока через границу `nsBoundary`, все без нового поля), верификатор
репортит каждую. Третьей (P/NP) границы нет —
...
- *Коллатц* (вне репо): затухающий дрейф, чья средняя сведена, а дно поглощающе.
