...
> доказал, что 4-канальная теорема выводится из 3-канальной вычёркиванием мёртвого
...
> (та же подпись, что `smallCleanSupply_iff_goal`). Плюс `repeated_level_signature_closes` — циркулярный
...
- **`lastStep00Obligation_of_energy`** через **`combinedEnergyProjection_resolves`** — энергетическая
  ...
  `Key × Energy`, а `no_double_spend_resolves` (при сожжённой альтернативе) — инъективность комбинированного
