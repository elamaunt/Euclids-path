...
> (первопричина `0 → 1` с четырьмя границами — twin-узел, римановский закон манифестации,
> НС-гейт-закон энергобаланса и закон каната Коллатца; карантин
> `Engine/CausalClosureAxiom.lean`, ровно 52 таких деклараций — 43 в карантине, 2 декларации
> геометрии гл. 49 (пересечение прямых из той же границы близнецов), 2 декларации дядического истока
> (насос каскада `n=0` через границу `nsBoundary`) и 5 деклараций Коллатца (закон каната через
> границу `collatzBoundary`), утечек нет — верификатор отслеживает каждую);
> 🔴 — открытый узел / цель.
...
**Первопричина — 🟡 карантин, четыре границы.** Единственная аксиома `step00FirstCause` несёт
twin-узел (`causalBoundary`), римановский закон манифестации (`riemannBoundary`, глава 38),
НС-гейт-закон энергобаланса (`nsBoundary`, глава 41 — добитие: пережил расширенную трилемму,
две ковки убиты гейтами; декрет, возможно, переплачивает — раскрыто) и закон каната Коллатца
(`collatzBoundary`, глава 56 — §18, знак эвристики благоприятный, декрет, возможно, переплачивает);
52 AXIOM-TAINTED декларации (42 в карантинном модуле + следствие `higherEnergyIncompatibility_twins`
+ 2 геометрии главы 49 через границу близнецов + 2 дядического истока через `nsBoundary` + 5 потребителя
Коллатца через новое поле `collatzBoundary`: `collatzRopeBoundary`, `collatz_from_firstCause`, две
растяжки, `collatz_locked_behind_engine_status`), верификатор пересчитывает каждую при сборке. Третьей (P/NP) границы нет —
...
буквально — на голом ядре Lean; главы 33–42 разворачивают первопричину (теперь с четырьмя
границами — близнецы, Риман, НС-гейт и закон каната Коллатца, главы 55–56) и классические фронты.
...
- *Коллатц:* затухающий дрейф, чья средняя сведена, а дно поглощающе; контрпример = вечный
  двигатель (хвост орбиты от минимума), и решить изнутри нельзя — только проверкой (главы 55–56).
