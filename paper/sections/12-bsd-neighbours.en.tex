...
The eighth mask, the Birch–Swinnerton-Dyer conjecture~\cite{clay-bsd}, enters the
...
Here it is not a prohibition but a tool. The Lean source is \srcpath{Engine/BSDFront.lean}; the core is
...
\begin{theorem}[descent has no engine]\label{thm:bsd_descent_has_no_engine}
\green{}\ On the rational point group of an elliptic curve the height descent has no perpetual
engine: for every height descent model, no infinite chain of points strictly decreasing under a
height function $h\colon W.\code{toAffine.Point}\to\mathbb{N}$ can exist.
\leantag{bsd\_descent\_has\_no\_engine}
\end{theorem}
...
\begin{theorem}[parity is the rank-parity node]\label{thm:bsd_parity_is_rankParity}
...
\leantag{bsd\_parity\_is\_rankParity}
\end{theorem}
...
\begin{definition}[weak BSD]\label{def:WeakBSD}
...
it is a named input, not a theorem. \leantag{WeakBSD}
\end{definition}
...
(\srcpath{Engine/BSDEpistemic.lean}): the proposed ground/beyondOwnHorizon pair degenerates, its second
...
honest \red{} openness remains. A contentful replacement (\srcpath{Engine/BSDAnalyticAnchor.lean})
...
\begin{theorem}[the diagonal self-correlation]\label{thm:chowlaCorrelation_zero_eq_card}
\green{}\ The diagonal $h=0$ yields perfect self-correlation:
$\sum_{n\le x}\lambda(n)^2=x$. \leantag{chowlaCorrelation\_zero\_eq\_card}
\end{theorem}
...
\begin{theorem}[the polynomial abc shadow]\label{thm:polynomial_abc_shadow}
...
\leantag{polynomial\_abc\_shadow}
\end{theorem}
...
\begin{theorem}[the polynomial Fermat--Catalan shadow]\label{thm:polynomial_fermat_catalan_shadow}
...
\leantag{polynomial\_fermat\_catalan\_shadow}
\end{theorem}
...
\begin{theorem}[the Mahler--Northcott shadow]\label{thm:mahler_northcott_shadow}
\green{}\ There are only finitely many polynomials of bounded degree and bounded Mahler measure. This
is \code{mathlib}'s \code{finite\_mahlerMeasure\_le}. \leantag{mahler\_northcott\_shadow}
\end{theorem}
...
\begin{theorem}[unbounded Landau primes are infinite]\label{thm:landauPrimes_infinite_of_unbounded}
...
even $k$. \leantag{landauPrimes\_infinite\_of\_unbounded}
\end{theorem}
