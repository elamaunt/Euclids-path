...
(\srcpath{EuclidsPath/Engine/EPMI.lean}, \srcpath{EuclidsPath/Engine/Irreversibility.lean},
\srcpath{EuclidsPath/Engine/UniversalEngine.lean})~\cite{lean4,euclidspath-repo}.
...
\begin{definition}[the descent step]\label{thm:DescentStep}
...
\begin{lemma}[the descent is strict]\label{thm:descent_strict}
\green{}\; If $1\le A$ and $\mathrm{DescentStep}\,A\,h\,h'$, then $h'<h$. \leantag{descent\_strict}
\end{lemma}
...
\begin{theorem}[no infinite descent]\label{thm:no_infinite_descent}
...
\end{equation}
From the existence of such a sequence, \code{False} is derived. \leantag{no\_infinite\_descent}
\end{theorem}
...
\begin{definition}[the engine state and step]\label{thm:State}
...
return. \leantag{State}
\end{definition}

\begin{theorem}[no perpetual engine]\label{thm:no_perpetual_engine}
...
\begin{theorem}[the boundary dichotomy]\label{thm:boundary_dichotomy}
...
\begin{theorem}[the engine never returns]\label{thm:engine_never_returns}
...
\end{equation}
\leantag{engine\_never\_returns}
\end{theorem}
...
\begin{theorem}[fuel ascent is strictly monotone]\label{thm:fuel_ascent_strictMono}
\green{}\; The map $n\mapsto n+1$ is strictly monotone: $\mathrm{StrictMono}\,(\lambda n.\,n+1)$.
\leantag{fuel\_ascent\_strictMono}
\end{theorem}
...
\begin{theorem}[a turned engine halts]\label{thm:turned_engine_halts}
...
\begin{definition}[a perpetual engine]\label{thm:PerpetualEngine}
...
\end{equation}
\leantag{PerpetualEngine}
\end{definition}

\begin{theorem}[engine iff not well-founded]\label{thm:perpetualEngine_iff_not_wellFounded}
...
\end{equation}
\leantag{perpetualEngine\_iff\_not\_wellFounded}
\end{theorem}
...
\begin{theorem}[no engine on a well-founded relation]\label{thm:no_perpetual_engine_of_wellFounded}
\green{}\; If $\mathrm{WellFounded}\,r$, then $\neg\,\mathrm{PerpetualEngine}\,r$.
\leantag{no\_perpetual\_engine\_of\_wellFounded}
\end{theorem}
...
\begin{theorem}[no engine under a decreasing rank]\label{thm:no_perpetual_engine_of_rank}
...
$\neg\,\mathrm{PerpetualEngine}\,r$. \leantag{no\_perpetual\_engine\_of\_rank}
\end{theorem}
...
\begin{theorem}[the engine operates on the reals]\label{thm:perpetualEngine_on_real}
\green{}\; On the continuum the engine operates:
$\mathrm{PerpetualEngine}\,(<)$ on $\R$. \leantag{perpetualEngine\_on\_real}
\end{theorem}
