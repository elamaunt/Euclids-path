...
> `prose/NN_*.md` (парная проза, 00→41) и модули `EuclidsPath/Engine/*.lean` (машинная проверка).
...
> `Engine/CausalClosureAxiom.lean`, ровно 38 таких деклараций, утечек нет — верификатор
...
ровно 37 AXIOM-TAINTED деклараций (36 в карантинном модуле + задокументированное следствие
`higherEnergyIncompatibility_twins`), верификатор репортит каждую. Третьей (P/NP) границы НЕТ —
намеренно и машинно (трилемма главы 39), четвёртой (Янг–Миллс) тоже (трилемма главы 40);
§11–§12 карантина фиксируют, что декрет УЖЕ говорит на языках P/NP и ЯМ: на своём масштабе
`A ≥ 5` он платит все сертификаты и отказывает поставке отклонений (гэпнут). Непротиворечивость
расширенной теории ⟺ неопровержимость узла и RH в базе (растяжки — узел / off-critical нуль /
¬RH / закон / несжимаемость на всех масштабах / поставка на всех масштабах); интернализация
...
буквально — на голом ядре Lean; главы 33–41 разворачивают первопричину (теперь с двумя
