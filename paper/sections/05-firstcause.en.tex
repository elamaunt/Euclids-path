...
brought in from outside. The same holds against a Hawking-radiation ``leak'': reconstructing the
interior from the outgoing radiation is an \emph{outside} operation (\code{ExternalUniverseCause}),
not an inside derivation, so it corroborates the boundary above rather than breaching it
(\code{hawking\_leaky\_horizon}); the physics is only a metaphor.
\end{remark}
