...
\begin{sidewaysfigure}
  \centering
  \includegraphics[width=0.95\textheight]{figures/navigator.en.pdf}
  \caption{Navigator map of the programme. One prohibition (\code{no\_infinite\_descent}); the main
  theorem \code{higherEnergyIncompatibility\_main}, green and axiom-free, \emph{pressing on} the single
  axiom (unknowable from within $\Rightarrow$ accept, do not derive); and every examined problem with its
  honest status --- \emph{taken} (and why not Collatz-attackable), \emph{deferred} (the heuristic sign),
  or \emph{fallen} (Collatz, refuted at $n=27$).}
  \label{fig:navigator}
\end{sidewaysfigure}
