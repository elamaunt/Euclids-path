...
- **`EuclidsPath/Engine/*.lean`** — основная (доказанная) линия: двигатель и его законы → редукция к
  ...
  хода доказательства**.
- **`prose/NN_*.md`** — парная проза, та же сквозная нумерация 00→44 (+ приложение `A`), единый
...
| 44 | **★ Кода: путь Евклида и структура пространства-времени** | [44](prose/44_Coda.md) | — (синтез) | философский/физический свод: один запрет — семь масок; возможная ТВ |
