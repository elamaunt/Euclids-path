...
**Definition (rank-(3,3) centre).** A centre `m` is called rank-(3,3) if there exist naturals `a\le b\le v` and `q\le r\le s` such that
...
**Theorem (`det_law_rank33`).** Let `1\le m`, `6m-1=abv` and `6m+1=qrs`. Then
$$q r s = a b v + 2.$$
...
**Definition (product coordinates).** For a rank-(3,3) centre set `C=av`, `D=qs`. Then the sides are written as `Cb=6m-1` and `Dr=6m+1`, where `b` and `r` are the remaining "free" factors.

**Theorem (`det_law_CD`).** Over `ℤ`: if `Cb=6m-1` and `Dr=6m+1`, then
$$C b - D r = -2.$$
...
**Definition (a ray and its step).** By a ray we mean a family of points with common `v,s`, where the i-th point satisfies the rank-one equation `u_i v+2=r_i s`, and the transition between two points is given by an integer step `K`:
$$u_2-u_1=sK.$$

**Theorem (`det_collapse`).** Let `u_1 v+2=r_1 s`, `u_2 v+2=r_2 s`, the step `u_2-u_1=sK` and `s\ne 0`. Then
$$u_2 r_1 - u_1 r_2 = 2K.$$
...
**Definition (Euclidean decomposition of a step).** Suppose the rank-one equation `abv+2=qP` holds. Decompose `P` and `ab` with respect to the scale `v`:
...
**Theorem (`euclid_descent_identity`).** Given `abv+2=qP`, `P=v^{2}+\Delta`, `ab=qv+\Xi`:
$$q\,\Delta - v\,\Xi = 2.$$
...
**Definition (the bridge equation and its shift).** Let the repeated atom `a` link two points by the bridge equations
$$a\,B_1=qs\,Q_1-2,\qquad a\,B_2=qs\,Q_2-2,$$
and let the shift along the bridge be a multiple of `6qs`:
$$B_2-B_1=6\,(qs)\,h.$$

**Theorem (`small_det_collapse`).** Given `a\ne 0`, `qs\ne 0` and the equations above:
