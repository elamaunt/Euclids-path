...
there is no way back --- the arrow of time is a theorem, not a postulate. Where that arrow bends most steeply reads as mass,
and the bending itself as gravity --- not a separate force but the curvature of the arrow of time,
computed as $\kappa$ (\cref{sec:geometry}); this curvature is moreover \emph{topological} --- its total
over any closed region is the Euler characteristic (\cref{thm:gaussBonnet_eq_euler}), so the bend is
book-kept by the shape of the descent alone and asks for no cosmological-constant term --- and the
$\pm$ twin pair sits on an axis \emph{orthogonal} to this time, its two members sharing one instant
under a fixed-time involution, charge conjugation rather than time reversal
(\cref{thm:lexRank_side_invariant}). Gravity and time are then two faces of one descent, its
curvature and its direction, though that we name them so is the interpretation. It has a beginning (the
...
bottom; a finite price of computation, Landauer's principle~\cite{landauer1961} in rank clothing (\cref{sec:pnp});
...
And this curvature is read off the numbers themselves, so it lies wherever numbers do --- under
language and knowledge, under matter examined closely, under the cosmos at large; the same
prohibition holds at every scale and in every guise, portable to any language that can count. By its
natural interdiction it draws a horizon, dividing what can be derived and known from within from the
first cause that can only be accepted from without --- the reachable and real from the metaphysical.
...
it would run the forbidden engine, not because the theory fails to decide it. With the well-foundedness
canon (\cref{sec:wellfounded}) in view this is sharpened, not softened: there the engine reaches
Goodstein's theorem and the Kirby--Paris hydra, which \emph{are} textbook natural instances of the
incompleteness phenomenon --- so ``not G\"odelian'' means precisely that the prohibition is a principle
of \emph{well-foundedness}, not of \emph{self-reference}. Those statements are independent because
``$\varepsilon_0$ is well-founded'' is what PA cannot prove (Gentzen), and the engine is the
well-founded machine \emph{behind} them, not a diagonal sentence.

Nor is the engine a decision procedure in disguise. It is a \emph{local} sieve: it may be pointed at
any statement, but it catches only where a deviation is genuinely a disguised infinite descent, and
each reduction is built by hand. No uniform procedure sifts all statements at once --- the provable
is recursively enumerable but not recursive~\cite{church1936,turing1936}; one may search, case by
case, for the descent behind a given question, but one cannot decide, once and for all, which
questions hide one. The reading is a lens for a class of problems reducible to descent, never an
oracle for all.
...
Read as time, this same divide settles a question it invites. The irreversible descent is the arrow
of time (\code{engine\_never\_returns}); yet on the well-founded line it cannot run forever, so there
time is a \emph{finite} descent --- a beginning at $0 \to 1$, a bottom it reaches --- and \emph{not} a
perpetual engine. Only on the continuum is a perpetual descent realised (\code{perpetualEngine\_on\_real}),
an eternal approach to a limit never touched. So ``is time a perpetual engine?'' is ``is time discrete
or continuous?'', and \code{universal\_engine\_dividing\_line} draws the answer: never perpetual where
well-foundedness holds, perpetual precisely where it fails. That the descent is time remains the
interpretation; the divide itself is proved.
