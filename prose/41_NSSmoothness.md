...
А сам закон не доказуем: для его вывода нужна теорема о дивергенции на `ℝ³`, которой в mathlib нет, — и не кован он никаким известным способом. Больше того, его опровержение решило бы открытую проблему Гальди о лиувиллевости стационарного Навье–Стокса. Это ровно тот эпистемический статус, что у twin-узла: не доказуем и не опровержим — а значит, честный кандидат в границы.
...
Но у этой границы есть особенность, которую надо назвать честно. У Римана декрет был *ровно* RH-силы: «закон ⟺ цель» при границе. Здесь такого зеркала нет.

Суррогату гладкости хватило бы даже *неравенства* (`noSingularCascade_of_twoTimeInequality` 🟢), а декрет утверждает более сильное *равенство* `dE/dt = −D`. Обратная импликация неизвестна — то есть **декрет, возможно, переплачивает** за свою цель. Это тот же паттерн, что мы честно отметим у Коллатца: цена может превышать стоимость.

Прежний вердикт §13 «пятой границы нет» остаётся в силе для безгейтных и манифестационных форм — `nsBoundary` выжил как *другой* кандидат, гейченный, с раскрытой переплатой.
...
Отдельно стоит наша находка добития — кованое давление. Поток `cookedFlow` набирает энергию из ничего (размешивается, `E = c·t²`), и это в точности портрет вечного двигателя: поле, качающее энергию без источника. У настоящего бессилового решения такое невозможно — но тотальные операторы mathlib (junk-производные) впустили самозванца через градиент давления на сфере. Гейт по дифференцируемости — это физическое требование «настоящей гладкости течения», изгоняющее самозванца. Так математическая честность (закрыть junk) и физическая интуиция (реальные течения гладки) оказываются одним и тем же жестом.
...
И как у Коллатца и у простых чисел, здесь стоит та же стена: в среднем каскад затухает — это мы видим, — но гарантия для *каждой* отдельной траектории остаётся открытым сердцем, именованным входом `EnergyBalanceLaw`, оплаченным декретом лишь под честно раскрытую переплату.
...
Итог: эта работа **не приближает** решение НС ни на шаг математически. Она даёт точную формулировку, проверенный скелет редукции и точное имя единственной недостающей теоремы — и честно показывает барьер, из-за которого та вне досягаемости. Ни `sorry`, ни новой аксиомы; таинт репозитория неизменен.
