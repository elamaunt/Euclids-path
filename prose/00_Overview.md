...
> `prose/NN_*.md` (парная проза, 00→50 плюс приложения A, B) и модули `EuclidsPath/Engine/*.lean`
> (машинная проверка).
...
(`collatzBoundary`, приложение F — §18, знак эвристики благоприятный, декрет, возможно, переплачивает);
...
границами — близнецы, Риман, НС-гейт и закон каната Коллатца, приложения E–F) и классические фронты.
...
- *Коллатц:* затухающий дрейф, чья средняя сведена, а дно поглощающе; контрпример = вечный
  двигатель (хвост орбиты от минимума), и решить изнутри нельзя — только проверкой (приложения E–F).
...
Восьмая задача тысячелетия — [Бёрч–Свиннертон-Дайер](C_BirchSwinnertonDyer.md) (приложение C) — входит
...
добавлено (трилемма). Такса — прежние 47.
