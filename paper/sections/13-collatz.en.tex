...
\emph{fuel injection} --- the ``$+1$'' in $3n+1$ raises the height.
\end{definition}
...
For odd $n\ge 1$ the accelerated step strictly raises the height: $n<\tfrac{3n+1}{2}=T(n)$. \green{}
\end{theorem}

\begin{theorem}[\code{not\_descent\_on\_odd}]\label{thm:not_descent_on_odd}
For odd $n\ge 1$ the step is not a descent, $\neg\,(T(n)<n)$. Hence the height along an orbit is not
\code{StrictAnti}, and \code{no\_infinite\_descent} does not apply. \green{}
\end{theorem}
...
Ascending positions are unbounded: for every $N$ there is $n>N$ with $n\equiv 1\ (\mathrm{mod}\ 4)$
and $n<(3n+1)/2$ (witness $n=4N+5$). Hence no monotone $\log_2$-height exists. \green{}
\end{theorem}
...
One move of the engine advances by at most a rank: $T(n)\le 2n$ for all $n$. \green{}
\end{theorem}

\begin{theorem}[\code{window\_budget}]\label{thm:window_budget}
Over a window of $k$ steps, of which $t=\mathrm{oddCount}(k,n)$ are odd and
...
bounds, and no logarithms are needed. \green{}
\end{theorem}

\begin{theorem}[\code{window\_descends}]\label{thm:window_descends}
If the rope wins the window strictly ($t<e$) and $n\ge 1$, then $T^{k}(n)<n$. \green{}
\end{theorem}
...
If $n\ge 1$ and $\mathrm{RopeCountingLaw}(n)$ holds, then $\exists K,\ T^{K}(n)=1$. \green{}
\end{theorem}
...
The orbit of $27$ reaches one in $70$ accelerated steps:
...
overall and never once leads from the start. \green{}
\end{theorem}

\emph{Proof idea.} The kernel closes the prefix up to $k=70$ by direct computation
(\code{counts\_le\_70\_at\_27}: $\mathrm{even}\le\mathrm{odd}$ in every prefix window;
\code{counts\_at\_70\_at\_27}: exactly $41$ against $29$, finishing at $1$). The tail falls into the
cycle $1\to 2\to 1$, where steps go strictly in pairs; the cycle lemma \code{cycle\_counts}, via the
additivity of the counters (\code{oddCount\_add}, \code{evenCount\_add}), freezes the deficit of
$-12$ forever, so no window from the start ever flips it.

\begin{theorem}[\code{not\_ropeCountingLaw\_27}]\label{thm:not_ropeCountingLaw_27}
The rope law is false for $n=27$: $\neg\,\mathrm{RopeCountingLaw}(27)$. From $j=0$ (value $27>2$) no
window gives a rope surplus. \green{}
\end{theorem}

\begin{theorem}[\code{ropeLaw\_universal\_refuted}]\label{thm:ropeLaw_universal_refuted}
The universal form of the law is false: $\neg\,\forall n\,(1\le n\Rightarrow\mathrm{RopeCountingLaw}(n))$,
with witness $n=27$ (\cref{thm:not_ropeCountingLaw_27}). A decree boundary on this law is impossible. \green{}
\end{theorem}
...
For $n\ge 3$ a non-descending orbit ($n\le T^{k}(n)$ for all $k$) loses no window to the rope
...
If an orbit never reaches $1$, then its tail from the global-minimum position is a non-descending,
never-halting orbit --- exhibited explicitly, not by contradiction. \green{}
\end{theorem}
...
A structure with two fields: \code{ground}, the law itself $\forall n\ge 1,\ \mathrm{RopeCountingLaw}(n)$,
and \code{beyondOwnHorizon}, a witness that it was derived by crossing the boundary,
...
Internal knowledge of the Collatz first cause is impossible: $\neg\,\mathrm{InternalKnowledgeOfCollatzCause}$.
...
A conjunction of three links: (1) infinite internal extraction of the rope law is impossible,
$\neg\,\mathrm{InternalKnowledgeOfCollatzCause}$; (2) the only internal trace of a counterexample is a
perpetual engine (\code{nonHalting\_carries\_perpetual\_engine}); (3) hence the solution is either
unknowable from inside, or accessible only by \emph{verification} of an orbit. \green{}
\end{theorem}
...
The epistemic status of Collatz after the decree's fall (the mirror of
...
(a refutation would build a perpetual engine). \green{}
\end{theorem}
...
For every $n\ge 1$ there is $K$ with $T^{K}(n)=1$; equivalently, every orbit has no perpetual tail.
\red{} Open, and untouched by the decree's fall: the refuted law was strictly stronger than the
