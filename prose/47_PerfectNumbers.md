...
разбирается `evenPerfect_eq` в форму `2^k·(2^(k+1)−1)`, а простота числа Мерсенна `2^(k+1) − 1`
...
Две открытые проблемы оказались буквально одной, и эта одинаковость — машинно-проверенная.
...
безэнергетически стабильную, а закон поставляет бесконечную семью потоков — не ex falso, а как
...
> бы карантин. Ни одного свободного Prop-поля, свободного гейта или переименованного вывода в модуле
...
целиком зелёный модуль, говорит две вещи. Первая: **предъявить нечётного свидетеля — значит
предъявить вечный двигатель**, и потому свидетель зелёно непредъявим
(`oddPerfectWitness_green_unpresentable` 🟢). Условность здесь названа условностью: обе гипотезы
...
(`oddPerfectWitness_carries_engine`), а двигателей зелёно нет (`no_someConcreteEuclideanEngine`).
...
откуда `oddPerfectCause_unknowable` 🟢: «узнать нельзя изнутри» — не лозунг, а теорема, зеркало
...
`no_oddPerfectWitness_below_horizon` 🟢 закрывает подвал безусловно. Сводка
...
> *самообоснования*, а не (не)существование нечётного совершенного числа. Третий конъюнкт
...
**Теорема** (`odd_perfect_euler_form`, 🟢). *Всякое нечётное совершенное `n` имеет вид `n = q^α · m²`,
...
не «какое-то нечётное число», а `q^α·m²` с `q ≡ α ≡ 1 (mod 4)`. И к этому портрету примыкают ещё два
...
— через оценку изобилия `2·∏(p−1) ≤ ∏p`). Три качественных обруча — `≥ 101`, `≥ 3` простых, форма
...
> он есть, — классику XVIII века, впервые встроенную в язык программы. Таинт 47 не меняется.
