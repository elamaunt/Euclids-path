...
> **Определение 28.1** (список больших факторов). Пусть `L : List ℕ` — список натуральных чисел. Скажем,
...
> **Определение 28.5** (`Clean A m`). Центр `m` чист относительно `A`, если ни одно простое `q ≤ A` не
...
> **Определение 28.8** (`AmbientLegal`, напоминание из главы 27). Семейство `factors : Fin r → ℕ`
...
- `hr4 : |L| ≤ 4` — из Теоремы 28.3 (`factor_rank_le_four`, §28.3), где посылка `∏ L ≤ 6X_A+1` получается
