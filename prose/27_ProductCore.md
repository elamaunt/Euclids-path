...
неконечность пространства подписей.

В этой главе мы устраняем все три *одним структурным ходом* — заменой объекта индукции.
Мы перестаём индуцировать по genealogies с их хвостом-абсорбером и переходим к чистому
**product-core**, где экстенсиональность становится настоящей теоремой, а не гипотезой.
...
1. **Экстенсиональность.** Чтобы «нет mismatch $\Rightarrow X_1 = X_2$», паспорт обязан
   ...
   принципа ящиков, дом-имя «пижонхол», см. [словарь](GLOSSARY.md))
...
*значит*: «нет mismatched role $\Rightarrow$ узлы равны» — теперь лемма, а не вход
(в дом-словаре вход/гейт — именованное недоказанное утверждение, см. [словарь](GLOSSARY.md)). Причина,
...
**Итог раздела.** Три fix встретились в одной точке: экстенсиональность (`no_mismatch_core_eq`) даёт знак-и-факторы
...
Напомним: Engine — вечный двигатель, главный запрещённый объект программы
(см. [словарь](GLOSSARY.md)); всё, что его строит, тем самым ложно.
...
машине, где три дефекта сняты структурно одним выбором объекта.

**Вывод.** Открытым остаётся ровно **один**
...
План: пункты (i)–(iii) — арифметика факторизации clean-стороны и
