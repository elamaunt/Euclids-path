...
\begin{theorem}[no gap means massless]\label{thm:not_massGap_iff_gapless}
...
\leantag{not\_massGap\_iff\_gapless}
\end{theorem}
...
\begin{theorem}[masslessness is a ladder]\label{thm:gapless_iff_nonempty_ladder}
...
$n$.
\leantag{gapless\_iff\_nonempty\_ladder}
\end{theorem}
...
\begin{definition}[quantization law]\label{def:QuantizationLaw}
\red{}\ A red per-model input (of the energy-balance pattern): every positive state carries a
...
\begin{theorem}[quantization gives the gap]\label{thm:massGap_of_quantizationLaw}
\green{}\ Quantization implies the mass gap: $\mathrm{QuantizationLaw}(S)\Rightarrow\mathrm{MassGap}(S)$.
\leantag{massGap\_of\_quantizationLaw}
\end{theorem}
...
\begin{theorem}[quantization is the gap verbatim]\label{thm:quantizationLaw_iff_massGap}
...
\leantag{quantizationLaw\_iff\_massGap}
\end{theorem}
...
\begin{theorem}[no unpaid descent chain]\label{thm:isEmpty_unpaidDescentChain}
...
\leantag{isEmpty\_unpaidDescentChain}
\end{theorem}
...
\begin{theorem}[descent law gives the conjecture]\label{thm:hodgeProperty_of_descentLaw}
\green{}\ If $\mathrm{DescentLaw}(S)$, then $\mathrm{HodgeProperty}(S)$: the descent law implies the
model's Hodge conjecture.
\leantag{hodgeProperty\_of\_descentLaw}
\end{theorem}
...
\begin{theorem}[no lawful unpaid model]\label{thm:no_unpaid_lawful_model}
...
exist.
\leantag{no\_unpaid\_lawful\_model}
\end{theorem}
...
\begin{theorem}[the law is the conjecture verbatim]\label{thm:descentLaw_iff_hodgeProperty}
\green{}\ For every ledger $S$: $\mathrm{DescentLaw}(S)\iff\mathrm{HodgeProperty}(S)$, green and with
no boundary anywhere.
\leantag{descentLaw\_iff\_hodgeProperty}
\end{theorem}
...
\begin{theorem}[the chain law is vacuous]\label{thm:hodgeChainManifestationLaw_green}
...
\leantag{hodgeChainManifestationLaw\_green}
\end{theorem}
...
\begin{theorem}[the manifestation form refutes the boundary]\label{thm:hodgeManifestationLaw_refutes_boundary}
...
\leantag{hodgeManifestationLaw\_refutes\_boundary}
\end{theorem}
...
\begin{theorem}[the Hodge cause is unknowable]\label{thm:hodgeCause_unknowable}
...
axioms.
\leantag{hodgeCause\_unknowable}
\end{theorem}
...
\begin{theorem}[Hodge locked behind the engine]\label{thm:hodge_locked_behind_engine_status}
\green{}\ For every ledger $S$:
...
\leantag{hodge\_locked\_behind\_engine\_status}
\end{theorem}
